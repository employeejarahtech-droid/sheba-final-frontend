...
export default function OperationTypes({ page, limit, search, setPage, setLimit, setSearch }: OperationTypesProps) {
    ...
    const can = useCan();
    const canCreate = can('indoor.master.operation-types.create');
    const canEdit = can('indoor.master.operation-types.edit');
    const canDelete = can('indoor.master.operation-types.delete');
    ...
    const deleteMutation = useMutation({
        mutationFn: async (id: number) => {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/operation-type/${id}`, {
                method: 'DELETE',
            ...
            render: (data: any, _type: string, row: OperationTypeItem) => {
                ...
                return `
                    <div class="flex items-center gap-2">
                        ...
                        <span class="font-mono text-xs text-purple-600 bg-purple-50 dark:bg-purple-950/30 dark:text-purple-400 px-2 py-1 rounded">${data.toString().startsWith('OT-') ? data : 'OT-' + data}</span>
                    </div>
                `;
            },
            defaultContent: "",
        ...
        <main className="p-4">
            <div className="space-y-4">
                ...
                <PageHeader
                    ...
                    actions={canCreate ? <CreateOperationTypeForm /> : null}
                />
                <DataTable
                    columns={columns}
