...
interface ClinicalBillsPageProps {
  page: number
  limit: number
  search: string
  from: string
...
export function ClinicalBillsPage({
  page,
  limit,
  search,
  from,
  ...
  // Clinical-service distributions resolve provider_id from the clinic_services
  // table (not doctors), so we filter by clinic service here.
  ...
  const selectedService = clinicServices.find(s => s.id === doctor)

  // ── Fetch ────────────────────────────────────────────────────────────────────
  const { data, isFetching } = useQuery<ApiResponse>({
    queryKey: ['clinical-bills', page, limit, search, from, to, status, doctor],
    queryFn: async () => {
      ...
      const res = await fetch(
        `${API_URL}/api/bill-distribution/final/clinical-service?${params.toString()}`,
        { headers: { Authorization: `Bearer ${token}` } }
      )
      if (!res.ok) throw new Error('Failed to fetch clinical bills')
      return res.json()
    },
    enabled: !!token,
  })
    ...
    <div className="flex flex-wrap items-end gap-2">
      <Popover open={doctorOpen} onOpenChange={setDoctorOpen}>
        <PopoverTrigger asChild>
          ...
          >
            <span className="truncate">
              {selectedService ? selectedService.name : 'Clinical Service'}
            </span>
        ...
        <PopoverContent className="w-72 p-0" align="start">
          <Command>
            <CommandInput placeholder="Search clinical service..." />
            <CommandList>
              <CommandEmpty>No clinical service found.</CommandEmpty>
              <CommandGroup>
                <CommandItem
                  value="all"
                ...
                >
                  ...
                  <span className="text-sm text-muted-foreground">All Clinical Services</span>
                </CommandItem>
                {clinicServices.map((s: any) => (
                  <CommandItem
                    key={s.id}
                    value={`${s.name} ${s.id}`.toLowerCase()}
                    onSelect={() => { setDoctor(s.id); setDoctorOpen(false) }}
                  >
                    <Check className={cn('mr-2 h-4 w-4', doctor === s.id ? 'opacity-100' : 'opacity-0')} />
      ...
      <main className="">
        ...
        <div className="flex flex-wrap items-end justify-between gap-2 mb-3">
          <h1 className="text-2xl font-bold tracking-tight flex items-center gap-2">
            <BriefcaseMedical className="h-6 w-6 text-violet-500" />
            Clinical Bills Management
          </h1>
        </div>
