...
interface ConsultantBillPageProps {
  page: number
  limit: number
  search: string
  from: string
...
// one consultant on the same admission (each gets its own bill_distribute_set_payable
// row and therefore its own distribution_id), so grouping by admission_id
// alone would incorrectly merge two different consultants' numbers into one
// row (confirmed this actually happens for Assistant-type distributions —
// same code path applies here). Falls back to grouping by the record's own
// id when distribution_id is null (a one-off record with no
// progressive-payment chain).
type AdmissionGroup = {
  key: number
  admission_id: number
...
export function ConsultantBillPage({
  page,
  limit,
  search,
  from,
  ...
  // ── Fetch doctors list (for Consultant filter) — only doctors with actual records here ──
  const { data: doctorsData } = useQuery({
    queryKey: ['doctors-list', 'consultant'],
    queryFn: async () => {
      const res = await fetch(`${API_URL}/api/bill-distribution/final/consultant/providers`, {
        headers: { Authorization: `Bearer ${token}` },
      })
  ...
  const { data, isFetching } = useQuery<ApiResponse>({
    queryKey: ['consultant-bills', page, limit, search, from, to, status, doctor],
    queryFn: async () => {
      ...
      const res = await fetch(
        `${API_URL}/api/bill-distribution/final/consultant?${params.toString()}`,
        { headers: { Authorization: `Bearer ${token}` } }
      )
      if (!res.ok) throw new Error('Failed to fetch consultant bills')
      return res.json()
    },
    enabled: !!token,
  })
  ...
  // progressive payment toward the same consultant-distribution is combined
  // into one row, but two different consultants on the same admission (each
  ...
  const createInvoiceMutation = useMutation({
    ...
    onSuccess: (invoice) => {
      ...
      setSelectedIds(new Set())
      queryClient.invalidateQueries({ queryKey: ['consultant-bills'] })
  ...
  // one allocation or more than one consultant. Indoor Management view keeps
  ...
  const flatColumns = useMemo(() => [
    ...
    ...(selectable ? [{
      ...
      render: (_: any, __: string, row: FinalDistribution) => {
        ...
        const title = alreadyPaid ? 'Already paid' : !doctor ? 'Filter by a specific consultant to select items' : ''
  ...
  const groupedColumns = useMemo(() => [
    ...
    },
    {
      ...
      render: (_: any, __: string, group: AdmissionGroup) => {
        ...
        // Each allocation to this admission's consultant distribution is its
            ...
            <CommandList>
              ...
              <CommandGroup>
                ...
                >
                  ...
                  <span className="text-sm text-muted-foreground">All Consultants</span>
                </CommandItem>
                {doctors.map((d: any) => (
                  <CommandItem
                    key={d.id}
  ...
  return (
    <>
      ...
      <main className="">
        ...
        {/* How-to hint (Finance flat view) — shown while no consultant is
        ...
        {payableOnly && selectable && !doctor && (
          <div className="flex items-start gap-2.5 mb-3 p-3 rounded-lg border bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-900">
            ...
            <p className="text-sm leading-relaxed text-blue-700 dark:text-blue-300">
              To generate a payment invoice, first select a consultant from the{' '}
              <strong>Consultant</strong> dropdown above the table — the row checkboxes stay
              disabled until a specific consultant is chosen, since one invoice can only
              contain a single consultant&rsquo;s bills. Once selected, check the boxes beside the
              bills you want to pay (or use <strong>Mark All</strong> to select every unpaid
              bill on the page — already-paid rows cannot be selected), verify the selected
              count and total, then click <strong>Create Payment Invoice</strong> to generate
