...
interface OtherBillsPageProps {
  ...
  to: string
  status: string
  /** Currently selected clinic-service provider id (0 = all). */
  service: number
...
export function OtherBillsPage({
  page,
  limit,
  search,
  from,
  ...
  // ── Fetch clinic services list (for provider filter) ─────────────────────────
  // "Other" distributions (bed/cabin charges and any charge not tied to a
  // surgeon/assistant/anesthetist/consultant/clinical-service provider) also
  // resolve provider_id from the clinic_services table — same as Clinical Bills.
  ...
  const clinicServices: any[] = clinicServicesData?.data?.items || clinicServicesData?.data?.rows || []
  const selectedService = clinicServices.find(s => s.id === service)

  // ── Fetch ────────────────────────────────────────────────────────────────────
  const { data, isFetching } = useQuery<ApiResponse>({
    queryKey: ['other-bills', page, limit, search, from, to, status, service],
    queryFn: async () => {
      ...
      if (service) params.set('doctor_id', String(service))
      const res = await fetch(
        `${API_URL}/api/bill-distribution/final/other?${params.toString()}`,
        { headers: { Authorization: `Bearer ${token}` } }
      )
      if (!res.ok) throw new Error('Failed to fetch clinic services bills')
      return res.json()
    },
    enabled: !!token,
  })
  ...
  const markPaidMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['other-bills'] })
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      render: (_: any, __: string, row: FinalDistribution) => {
        const name = row.service_name || row.doctor?.doctor_name || '—'
        const note = row.notes ? `<span class="text-xs text-muted-foreground">${row.notes}</span>` : ''
        return `
  ...
  const filterSlot = (
    <div className="flex flex-wrap items-end gap-2">
      <Popover open={serviceOpen} onOpenChange={setServiceOpen}>
        <PopoverTrigger asChild>
          <Button
            variant="outline"
            role="combobox"
            className={cn('h-9 text-sm w-56 justify-between font-normal', !service && 'text-muted-foreground')}
          >
            <span className="truncate">
              {selectedService ? selectedService.name : 'Clinic Service'}
            </span>
        ...
        <PopoverContent className="w-72 p-0" align="start">
          <Command>
            <CommandInput placeholder="Search clinic service..." />
            <CommandList>
              <CommandEmpty>No clinic service found.</CommandEmpty>
              <CommandGroup>
                <CommandItem
                  value="all"
                  onSelect={() => { setService(0); setServiceOpen(false) }}
                >
                  <Check className={cn('mr-2 h-4 w-4', !service ? 'opacity-100' : 'opacity-0')} />
                  <span className="text-sm text-muted-foreground">All Clinic Services</span>
                </CommandItem>
                {clinicServices.map((s: any) => (
                  <CommandItem
                    key={s.id}
                    value={`${s.name} ${s.id}`.toLowerCase()}
                    onSelect={() => { setService(s.id); setServiceOpen(false) }}
                  >
                    <Check className={cn('mr-2 h-4 w-4', service === s.id ? 'opacity-100' : 'opacity-0')} />
      ...
      {(from || to || status !== 'all' || service) && (
        <Button
          variant="ghost"
          size="sm"
          className="h-9 text-xs text-muted-foreground"
          onClick={() => { setFrom(''); setTo(''); setStatus('all'); setService(0) }}
        >
          Clear
        </Button>
      )}
      ...
      <main className="">
        ...
        <div className="flex flex-wrap items-end justify-between gap-2 mb-3">
          <h1 className="text-2xl font-bold tracking-tight flex items-center gap-2">
            ...
            Clinic Services Bills Management
          </h1>
        </div>
        ...
        <div onClick={handleTableClick}>
          <DataTable
            ...
            tableTitle="Clinic Services Bills"
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="rounded-xl bg-gradient-to-br from-violet-50 to-blue-50 dark:from-violet-950/30 dark:to-blue-950/30 border p-4 text-sm space-y-2">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Clinic Service</span>
