...
interface SurgeonBillPageProps {
  page: number
  limit: number
  search: string
  from: string
...
// one surgeon on the same admission (each gets its own bill_distribute_set_payable
// row and therefore its own distribution_id), so grouping by admission_id
// alone would incorrectly merge two different surgeons' numbers into one row
// (confirmed this actually happens for Assistant-type distributions — same
// code path applies here). Falls back to grouping by the record's own id
// when distribution_id is null (a one-off record with no progressive-payment
// chain).
type AdmissionGroup = {
  key: number
  admission_id: number
...
export function SurgeonBillPage({
  page,
  limit,
  search,
  from,
  ...
  // ── Fetch doctors list (for Surgeon filter) — only doctors with actual records here ──
  const { data: doctorsData } = useQuery({
    queryKey: ['doctors-list', 'surgeon'],
    queryFn: async () => {
      const res = await fetch(`${API_URL}/api/bill-distribution/final/surgeon/providers`, {
        headers: { Authorization: `Bearer ${token}` },
      })
  ...
  const { data, isFetching } = useQuery<ApiResponse>({
    queryKey: ['surgeon-bills', page, limit, search, from, to, status, doctor],
    queryFn: async () => {
      ...
      if (!res.ok) throw new Error('Failed to fetch surgeon bills')
      return res.json()
    },
    enabled: !!token,
  })
  ...
  // progressive payment toward the same surgeon-distribution is combined
  // into one row, but two different surgeons on the same admission (each
  ...
  const createInvoiceMutation = useMutation({
    ...
    onSuccess: (invoice) => {
      ...
      setSelectedIds(new Set())
      queryClient.invalidateQueries({ queryKey: ['surgeon-bills'] })
  ...
  // one allocation or more than one surgeon. Indoor Management view keeps the
  // grouped-by-distribution "Payment History" card layout.
  const flatColumns = useMemo(() => [
    ...
    ...(selectable ? [{
      ...
      render: (_: any, __: string, row: FinalDistribution) => {
        ...
        const title = alreadyPaid ? 'Already paid' : !doctor ? 'Filter by a specific surgeon to select items' : ''
  ...
  const groupedColumns = useMemo(() => [
    ...
    },
    {
      ...
      render: (_: any, __: string, group: AdmissionGroup) => {
        ...
        // Each allocation to this admission's surgeon distribution is its
            ...
            <CommandList>
              ...
              <CommandGroup>
                ...
                >
                  ...
                  <span className="text-sm text-muted-foreground">All Surgeons</span>
                </CommandItem>
                {doctors.map((d: any) => (
                  <CommandItem
                    key={d.id}
  ...
  return (
    <>
      ...
      <main className="">
        ...
        {/* How-to hint (Finance flat view) — shown while no surgeon is chosen;
        ...
        {payableOnly && selectable && !doctor && (
          <div className="flex items-start gap-2.5 mb-3 p-3 rounded-lg border bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-900">
            ...
            <p className="text-sm leading-relaxed text-blue-700 dark:text-blue-300">
              To generate a payment invoice, first select a surgeon from the{' '}
              <strong>Surgeon</strong> dropdown above the table — the row checkboxes stay
              disabled until a specific surgeon is chosen, since one invoice can only contain
              a single surgeon&rsquo;s bills. Once selected, check the boxes beside the bills
