...
export default function MyInvoices() {
    ...
    const { data } = useQuery({
        queryKey: ["my-invoices", page, limit, search, statusFilter],

        queryFn: async () => {
            const statusParam = statusFilter !== "all" ? `&status=${statusFilter}` : "";
            const res = await fetch(
                `${import.meta.env.VITE_API_URL}/api/outdoor-invoice/my-invoices?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}${statusParam}`,
                {
