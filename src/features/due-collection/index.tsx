...
export default function DueCollection() {
  ...
  const { data } = useQuery({
    queryKey: ["due-invoices", page, limit, search],
    queryFn: async () => {
  ...
  const columns = [
    {
      ...
      render: (data: any, _type: string, row: InvoiceItem) => {
        const display = data ? `<span class="font-semibold text-orange-600">${data}</span>` : '<span class="text-muted-foreground text-sm">-</span>';
        return `
