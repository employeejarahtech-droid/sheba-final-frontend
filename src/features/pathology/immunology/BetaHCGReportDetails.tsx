...
interface BetaHCGReportDetailsProps {
    hcgData: {
        id: number;
        invoice_id: number;
        test_date?: string;
...
export default function BetaHCGReportDetails({ hcgData, invoiceData, paddingTop = 40, fontSize = 1, showSignature = true }: BetaHCGReportDetailsProps) {
    ...
    return (
        ...
        `}
            ...
            <table className="w-full text-sm mt-6">
                ...
                <tbody>
                    <tr className={`border-b-${borderWidth} border-dashed border-b`}>
                        <td className="px-3 py-2">Beta HCG Level</td>
                        <td className="px-3 py-2">{hcgData?.hcg_level || 'Pending'}</td>
                        <td className="px-3 py-2">&lt;5 mIU/mL (Non-pregnant)</td>
                    </tr>
                    <tr className={`border-b-${borderWidth} border-dashed border-b`}>
                        <td className="px-3 py-2 font-semibold">Result</td>
                        <td className="px-3 py-2 font-semibold">{hcgData?.result || 'Pending'}</td>
                        <td className="px-3 py-2">Positive, Negative</td>
                    </tr>
                    {hcgData?.remarks && (
                        <tr className={`border-b-${borderWidth} border-dashed border-b`}>
                            <td className="px-3 py-2">Remarks</td>
                            <td className="px-3 py-2" colSpan={2}>{hcgData.remarks}</td>
                        </tr>
                    )}
                </tbody>
            </table>
            ...
            <p className="text-sm mt-4">
                ...
                {hcgData?.test_carried_out_by || 'Not specified'}
            </p>
