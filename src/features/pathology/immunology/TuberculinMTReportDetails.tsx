...
interface TuberculinMTReportDetailsProps {
    mtData: {
        id: number;
        invoice_id: number;
        test_date?: string;
        induration_mm: string;
        result: string;
        remarks?: string;
...
export default function TuberculinMTReportDetails({ mtData, invoiceData }: TuberculinMTReportDetailsProps) {
            ...
            <table className="w-full text-sm mt-6">
                ...
                <tbody>
                    <tr className={`border-b-${borderWidth} border-dashed border-b`}>
                        <td className="px-3 py-2">Induration</td>
                        <td className="px-3 py-2">{mtData?.induration_mm ? `${mtData.induration_mm} mm` : 'Pending'}</td>
                        <td className="px-3 py-2">0-15 mm</td>
                    </tr>
                    <tr className={`border-b-${borderWidth} border-dashed border-b`}>
                        <td className="px-3 py-2 font-semibold">Result</td>
                        <td className="px-3 py-2 font-semibold">{mtData?.result || 'Pending'}</td>
                        <td className="px-3 py-2">Positive, Negative, Doubtful</td>
                    </tr>
                    {mtData?.remarks && (
                        <tr className={`border-b-${borderWidth} border-dashed border-b`}>
                            <td className="px-3 py-2">Remarks</td>
                            <td className="px-3 py-2" colSpan={2}>{mtData.remarks}</td>
                        </tr>
                    )}
                </tbody>
            </table>
