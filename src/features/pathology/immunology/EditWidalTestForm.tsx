    ...
    useEffect(() => {
        if (widalTestData) {
            form.reset({
                ...
                status: String(widalTestData.status).trim().toLowerCase() === 'complete' ? 'complete' : 'incomplete',
            })
        }
    }, [widalTestData]);
            ...
            <SheetContent className="max-w-[450px] w-full overflow-y-auto">
                ...
                <div className="px-4">
                    <PatientInvoiceInfo invoiceInfo={{
                        invoiceNo: widalTestData?.invoice_id ? `RPT-${widalTestData.invoice_id}` : "—",
                        patientName: widalTestData?.outdoor_invoice?.patient_name || "—",
                        age: widalTestData?.outdoor_invoice?.age_text || widalTestData?.outdoor_invoice?.age || "—",
                        gender: widalTestData?.outdoor_invoice?.sex || "—",
                    }} />
                </div>
