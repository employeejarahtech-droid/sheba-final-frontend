...
interface UrineForAlbuminReportDetailsProps {
    urineAlbuminData: {
        id: number;
        invoice_id: number;
        albumin: string;
...
export default function UrineForAlbuminReportDetails({ urineAlbuminData, invoiceData, paddingTop = 40, fontSize = 1, showSignature = true }: UrineForAlbuminReportDetailsProps) {
    ...
    return (
        ...
        `}
            ...
            <table className="w-full text-sm mt-6">
                ...
                <tbody>
                    <tr className={`border-b-${borderWidth} border-dashed border-b`}>
                        <td className="px-3 py-2">Albumin</td>
                        <td className="px-3 py-2 font-semibold">{urineAlbuminData?.albumin || 'Pending'}</td>
                        <td className="px-3 py-2">Negative</td>
                    </tr>
                    {urineAlbuminData?.remarks && (
                        <tr className={`border-b-${borderWidth} border-dashed border-b`}>
                            <td className="px-3 py-2">Remarks</td>
                            <td className="px-3 py-2" colSpan={2}>{urineAlbuminData.remarks}</td>
                        </tr>
                    )}
                </tbody>
            </table>
            ...
            <p className="text-sm mt-4">
                ...
                {urineAlbuminData?.test_carried_out_by || 'Not specified'}
            </p>
