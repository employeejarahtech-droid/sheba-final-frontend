...
interface ReducingSubstanceReportDetailsProps {
    reducingSubstanceData: {
        id: number;
        invoice_id: number;
        test_result: string;
        remarks?: string;
...
export default function ReducingSubstanceReportDetails({ reducingSubstanceData, invoiceData }: ReducingSubstanceReportDetailsProps) {
            ...
            <table className="w-full text-sm mt-6">
                ...
                <tbody>
                    <tr className={`border-b-${borderWidth} border-dashed border-b`}>
                        ...
                        <td className="px-3 py-2 font-semibold">{reducingSubstanceData?.test_result || 'Pending'}</td>
                        <td className="px-3 py-2">Negative</td>
                    </tr>
                    {reducingSubstanceData?.remarks && (
                        <tr className={`border-b-${borderWidth} border-dashed border-b`}>
                            <td className="px-3 py-2">Remarks</td>
                            <td className="px-3 py-2" colSpan={2}>{reducingSubstanceData.remarks}</td>
                        </tr>
                    )}
                </tbody>
            </table>
