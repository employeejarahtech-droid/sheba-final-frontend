...
export default function OutdoorAllCollections({
    ...
    // Fetch departments for mapping department ID to department name
        ...
        },
        {
            ...
            render: (_d: any, _t: string, row: any) =>
                `<div class="flex items-center gap-2">
                    ...
                    <button onclick="window.location.href = '/dashboard/outdoor/reception/invoices/${row.id}'"
    ...
    const buildInvoiceDetailHTML = (inv: any) => {
        ...
        const paymentsHTML = inv.payments?.map((p: any) =>
            `<tr class="border-b"><td class="py-2 px-3">${fmtDateTime(p.payment_date || p.created_at)}</td><td class="py-2 px-3">${p.method || p.payment_method || '-'}</td><td class="py-2 px-3">${p.creator?.name || p.created_by_user?.name || '-'}</td><td class="py-2 px-3 text-right text-emerald-600 font-medium">${fmtNum(p.amount)}</td></tr>`
        ).join('') || ''

        const discountsHTML = inv.department_discounts?.map((d: any) => {
            const dept = departments.find((dept: any) => Number(dept.id) === Number(d.department_id))
            const deptName = d.department?.name || d.department_name || d.department || dept?.name || `Department ID: ${d.department_id}` || 'Unknown'
            return `<tr class="border-b"><td class="py-2 px-3">${deptName}</td><td class="py-2 px-3">${d.creator?.name || d.created_by_user?.name || '-'}</td><td class="py-2 px-3 text-right text-orange-600 font-medium">${fmtNum(d.discount)}</td></tr>`
        }).join('') || ''
        ...
        <div class="grid grid-cols-2 gap-x-8 gap-y-4 text-sm border-b pb-6 mb-4">
            ...
            <div><p class="text-gray-500">Bill Created By</p><p class="font-semibold text-gray-800">${inv.creator?.name || inv.created_by_user?.name || inv.created_by || '-'}</p></div>
        </div>
        ...
        ${discountsHTML ? `<div class="mb-4"><h3 class="text-lg font-semibold mb-3 text-gray-800">Discount History</h3><table class="w-full text-sm border"><thead><tr class="bg-gray-50"><th class="py-2 border text-left px-3">Department</th><th class="py-2 border text-left px-3">Created By</th><th class="py-2 border text-right px-3 w-24">Amount</th></tr></thead><tbody>${discountsHTML}</tbody></table></div>` : ''}
        ${paymentsHTML ? `<div class="mb-4"><h3 class="text-lg font-semibold mb-3 text-gray-800">Payment History</h3><table class="w-full text-sm border"><thead><tr class="bg-gray-50"><th class="py-2 border text-left px-3">Date</th><th class="py-2 border text-left px-3">Method</th><th class="py-2 border text-left px-3">Created By</th><th class="py-2 border text-right px-3 w-24">Amount</th></tr></thead><tbody>${paymentsHTML}</tbody></table></div>` : ''}
