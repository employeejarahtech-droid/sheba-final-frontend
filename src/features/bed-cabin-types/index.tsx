...
export default function BedCabinTypes({ page, limit, search, setPage, setLimit, setSearch }: BedCabinTypesProps) {
    ...
    const can = useCan();
    const canCreate = can('indoor.master.bed-cabin-types.create');
    const canEdit = can('indoor.master.bed-cabin-types.edit');
    const canDelete = can('indoor.master.bed-cabin-types.delete');
    ...
    const deleteMutation = useMutation({
        mutationFn: async (id: number) => {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/bed-cabin-type/${id}`, {
                method: 'DELETE',
            ...
            render: (data: any, _type: string, row: BedCabinTypeItem) => {
                ...
                return `
                    <div class="flex items-center gap-2">
                        ...
                        <span class="font-mono text-xs text-purple-600 bg-purple-50 dark:bg-purple-950/30 dark:text-purple-400 px-2 py-1 rounded">${data.toString().startsWith('BCT-') ? data : 'BCT-' + data}</span>
                    </div>
                `;
            },
        },
        ...
        <main className="p-4">
            <div className="space-y-4">
                ...
                <PageHeader
                    ...
                    actions={canCreate ? <CreateBedCabinTypeForm /> : null}
                />
                <DataTable
                    columns={columns}
