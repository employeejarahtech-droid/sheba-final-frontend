...
export default function DoctorTypes({ page, limit, search, setPage, setLimit, setSearch }: DoctorTypesProps) {
    ...
    const deleteMutation = useMutation({
        mutationFn: async (id: number) => {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/doctor-type/${id}`, {
                method: 'DELETE',
    ...
    const { data } = useQuery({
        queryKey: ["doctor-types", page, limit, search],
        queryFn: async () => {
        ...
        const handleExpandClick = (e: Event) => {
            ...
            cardContainer.innerHTML = `
                <div class="max-w-3xl mx-auto bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                    <!-- Header -->
                    <div class="bg-gradient-to-r from-purple-600 to-fuchsia-600 px-6 py-4">
                        <h2 class="text-lg font-semibold text-white">Doctor Type Information</h2>
                        <p class="text-purple-100 text-sm">Detailed overview of selected doctor type</p>
                    </div>
                    ...
                    <div class="p-6">
                        <ul class="grid md:grid-cols-2 gap-6 text-sm">
                            ...
                            <li class="flex flex-col">
                                <span class="text-gray-500">Doctor Type ID</span>
            ...
            render: (data: any, _type: string, row: DoctorTypeItem) => {
                ...
                return `
                    <div class="flex items-center gap-2">
                        ...
                        <span class="font-mono text-xs text-purple-600 bg-purple-50 dark:bg-purple-950/30 dark:text-purple-400 px-2 py-1 rounded">${data.toString().startsWith('DT-') ? data : 'DT-' + data}</span>
                    </div>
                `;
            },
        },
