...
import { EditServiceCategoryForm } from './components/EditServiceCategoryForm'
...
export default function ServiceCategories({ page, limit, search, setPage, setLimit, setSearch }: ServiceCategoriesProps) {
    ...
    const can = useCan();
    const canCreate = can('indoor.master.service-categories.create');
    const canEdit = can('indoor.master.service-categories.edit');
    const canDelete = can('indoor.master.service-categories.delete');
    ...
    const deleteMutation = useMutation({
        mutationFn: async (id: number) => {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/service-category/${id}`, {
                method: 'DELETE',
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries({ queryKey: ['service-categories-overall-stats'] });
        },
    ...
    const { data } = useQuery({
        queryKey: ["service-categories", page, limit, search],
        queryFn: async () => {
            const res = await fetch(
                `${import.meta.env.VITE_API_URL}/api/service-category?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}`,
                {
                    headers: { Authorization: `Bearer ${token}` },
                }
            );
    ...
    const { data: statsData } = useQuery({
        queryKey: ["service-categories-overall-stats"],
        queryFn: async () => {
            const res = await fetch(
                `${import.meta.env.VITE_API_URL}/api/service-category/statistics/overall`,
                {
                    headers: { Authorization: `Bearer ${token}` },
                }
            );
            if (!res.ok) return { totalServiceCategories: 0, totalServices: 0 };
            const result = await res.json();
            return result.data || { totalServiceCategories: 0, totalServices: 0 };
        },
        enabled: !!token,
    });

    const stats = statsData || { totalServiceCategories: 0, totalServices: 0 };

    const statCards = [
        {
            title: 'Total Service Categories',
            value: stats.totalServiceCategories || 0,
            icon: Folder,
    ...
    const columns = [
        {
            ...
            render: (data: any, _type: string, row: ServiceCategoryItem) => {
                ...
                return `
                    <div class="flex items-center gap-2">
                        ...
                        <span class="font-mono text-xs text-purple-600 bg-purple-50 dark:bg-purple-950/30 dark:text-purple-400 px-2 py-1 rounded">${data.toString().startsWith('SC-') ? data : 'SC-' + data}</span>
                    </div>
                `;
            },
            defaultContent: "",
        ...
        },
        {
            data: "created_by",
            title: "Created By",
            render: (_data: any, _type: string, row: ServiceCategoryItem) => {
            ...
            cardContainer.innerHTML = `
                <div class="max-w-3xl mx-auto bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                    ...
                    <div class="bg-gradient-to-r from-pink-600 to-rose-600 px-6 py-4">
                        <h2 class="text-lg font-semibold text-white">Service Category Information</h2>
                        <p class="text-pink-100 text-sm">Detailed overview of selected service category</p>
                    </div>
                    ...
                    <div class="p-6">
                        ...
                        <div class="mt-8 flex justify-end gap-3 border-t pt-5">
                            ...
                            ${canEdit ? `<button data-action="edit" data-id="${id}"
                                    class="inline-flex items-center justify-center rounded-lg text-sm font-medium bg-pink-600 text-white hover:bg-pink-700 transition h-10 px-5 shadow">
                                Edit
                            </button>` : ''}
                        </div>
                    </div>
        ...
        <main className="p-4">
            <div className="space-y-4">
                ...
                <PageHeader
                    ...
                    actions={canCreate ? <CreateServiceCategoryForm /> : null}
                />
                <DataTable
                    columns={columns}
            ...
            <EditServiceCategoryForm open={openEditForm} setOpen={setOpenEditForm} serviceCategoryId={selectedServiceCategoryId} />
        </main>
    </>
}
