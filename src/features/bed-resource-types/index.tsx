...
import { CreateBedResourceTypeForm } from './components/CreateBedResourceTypeForm'
import { EditBedResourceTypeForm } from './components/EditBedResourceTypeForm'
...
interface BedResourceTypesProps {
    page: number;
    limit: number;
    search: string;
...
export default function BedResourceTypes({ page, limit, search, setPage, setLimit, setSearch }: BedResourceTypesProps) {
    const [openEditForm, setOpenEditForm] = useState<boolean>(false);

    const can = useCan();
    const canCreate = can('indoor.master.resource-types.create');
    const canEdit = can('indoor.master.resource-types.edit');
    const canDelete = can('indoor.master.resource-types.delete');
    const [selectedBedResourceTypeId, setSelectedBedResourceTypeId] = useState<number | null>(null);

    const token = getCookie('accessToken');
    const queryClient = useQueryClient();
    const deleteMutation = useMutation({
        mutationFn: async (id: number) => {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/bed-resource-type/${id}`, {
                method: 'DELETE',
    ...
    const { data } = useQuery({
        queryKey: ["bed-resource-types", page, limit, search],
        queryFn: async () => {
            const res = await fetch(
                `${import.meta.env.VITE_API_URL}/api/bed-resource-type?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}`,
                {
                    headers: { Authorization: `Bearer ${token}` },
                }
            );
            if (!res.ok) throw new Error("Failed to fetch bed resource types");
            return res.json();
        },
        enabled: !!token,
    ...
    const { data: statsData } = useQuery({
        queryKey: ["bed-resource-types-overall-stats"],
        queryFn: async () => {
            const res = await fetch(
                `${import.meta.env.VITE_API_URL}/api/bed-resource-type/statistics/overall`,
                {
                    headers: { Authorization: `Bearer ${token}` },
                }
            );
            if (!res.ok) return { totalResourceTypes: 0, totalBeds: 0 };
            const result = await res.json();
            return result.data || { totalResourceTypes: 0, totalBeds: 0 };
        },
        enabled: !!token,
    });

    const stats = statsData || { totalResourceTypes: 0, totalBeds: 0 };

    const statCards = [
        {
            title: 'Total Resource Types',
            value: stats.totalResourceTypes || 0,
            icon: Layers,
            ...
            cardContainer.innerHTML = `
                <div class="max-w-3xl mx-auto bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                    ...
                    <div class="bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-4">
                        <h2 class="text-lg font-semibold text-white">Bed Resource Type Information</h2>
                        <p class="text-blue-100 text-sm">Detailed overview of selected bed resource type</p>
                    </div>
                    ...
                    <div class="p-6">
                        <ul class="grid md:grid-cols-2 gap-6 text-sm">
                            ...
                            <li class="flex flex-col">
                                <span class="text-gray-500">Resource Type ID</span>
    ...
    const columns = [
        {
            data: "id",
            title: "ID",
            render: (data: any, _type: string, row: BedResourceTypeItem) => {
                ...
                return `
                    <div class="flex items-center gap-2">
                        ...
                        <span class="font-mono text-xs text-purple-600 bg-purple-50 dark:bg-purple-950/30 dark:text-purple-400 px-2 py-1 rounded">${data.toString().startsWith('RT-') ? data : 'RT-' + data}</span>
                    </div>
                `;
            },
        },
    ...
    useEffect(() => {
        const handleTableClick = (e: Event) => {
            ...
            if (action === 'edit' && id) {
                setSelectedBedResourceTypeId(Number(id));
    ...
    }, [setSelectedBedResourceTypeId, setOpenEditForm]);
    ...
    return <>
        ...
        <main className="p-4">
            <div className="space-y-4">
                ...
                {/* Header & Table */}
                <PageHeader
                    title="List of Bed Resource Types"
                    actions={canCreate ? <CreateBedResourceTypeForm /> : null}
                />
                <DataTable
                    columns={columns}
            ...
            <EditBedResourceTypeForm open={openEditForm} setOpen={setOpenEditForm} bedResourceTypeId={selectedBedResourceTypeId} />
        </main>
    </>
}
