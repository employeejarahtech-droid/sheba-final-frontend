...
export default function PatientTypes({ page, limit, search, setPage, setLimit, setSearch }: PatientTypesProps) {
    ...
    const can = useCan();
    const canCreate = can('indoor.master.patient-types.create');
    const canEdit = can('indoor.master.patient-types.edit');
    const canDelete = can('indoor.master.patient-types.delete');
    ...
    const deleteMutation = useMutation({
        mutationFn: async (id: number) => {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/patient-type/${id}`, {
                method: 'DELETE',
            ...
            render: (data: any, _type: string, row: PatientTypeItem) => {
                ...
                return `
                    <div class="flex items-center gap-2">
                        ...
                        <span class="font-mono text-xs text-purple-600 bg-purple-50 dark:bg-purple-950/30 dark:text-purple-400 px-2 py-1 rounded">${data.toString().startsWith('PT-') ? data : 'PT-' + data}</span>
                    </div>
                `;
            },
        },
    ...
    }, [navigate, setSelectedPatientTypeId, setOpenEditForm]);
        ...
        <main className="p-4">
            <div className="space-y-4">
                ...
                <PageHeader
                    ...
                    actions={canCreate ? <CreatePatientTypeForm /> : null}
                />
                <DataTable
                    columns={columns}
