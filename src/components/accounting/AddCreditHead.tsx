...
import { useCreateIncomeHeadMutation } from "@/features/accounting/accountingQueries";
...
export default function AddCreditHeadForm() {
  ...
  const canCreateCreditHeads = true;
  ...
  const createIncomeHeadMutation = useCreateIncomeHeadMutation();
  const isLoading = createIncomeHeadMutation.isPending;
  ...
  const handleAddCreditHead = async (
    ...
    try {
      await createIncomeHeadMutation.mutateAsync(payload);
      toast.success("Credit Head added successfully");
      setOpen(false);
      form.reset();
    } catch (error) {
