...
import { useGetSingleCreditHeadQuery, useUpdateCreditHeadMutation } from "@/features/accounting/accountingQueries";
...
export default function EditCreditHeadForm({
  ...
  // TODO: Add permissions when auth store is available
  const canEditCreditHeads = true;
  ...
  const updateCreditHead = useUpdateCreditHeadMutation();
  const isLoading = updateCreditHead.isPending;
  ...
  const handleAddCreditHead = async (
    ...
    try {
      await updateCreditHead.mutateAsync(payload);
      console.log("Credit Head updated successfully");
      toast.success("Credit Head updated successfully");
      setOpen(false);
      form.reset();
    } catch (error) {
      console.error("Failed to update Credit Head:", error);
      toast.error("Failed to update Credit Head");
    }
  };
