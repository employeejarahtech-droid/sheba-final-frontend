...
export default function ChartOfAccounts() {
    ...
    const onSubmit = async (values: CreateAccountFormValues) => {
        ...
        try {
            if (editingAccount) {
                await updateAccountingAccount.mutateAsync({ id: editingAccount.id, body: payload });
                toast.success("Account updated successfully");
            } else {
                await addAccountingAccount.mutateAsync(payload);
                    ...
                    <Popover open={open} onOpenChange={setOpen}>
                        ...
                        <PopoverContent className="w-full p-0">
                            <Command>
                                ...
                                <CommandGroup>
                                    {searchAccounts.length === 0 ? <CommandItem disabled>Loading...</CommandItem> :
