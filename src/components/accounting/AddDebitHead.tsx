...
export default function AddDebitHeadForm() {
  ...
  // TODO: Fix permission checks when config/permissions is available
  ...
  const canCreateDebitHeads = true;
  ...
  const addDebitHead = useCreateExpanseHeadMutation();
  const isLoading = addDebitHead.isPending;
  ...
  const handleAddCreditHead = async (
    ...
    try {
      const res = await addDebitHead.mutateAsync(payload);
      console.log("Debit Head added successfully:", res);

      toast.success("Debit Head added successfully");
        setOpen(false);
        form.reset();
    } catch (error) {
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild>
        <button className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-500 px-5 py-2.5 font-medium text-white shadow-lg shadow-blue-500/20 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-blue-500/40 active:translate-y-0 active:shadow-none">
          <PlusCircle size={18} />
          Add Debit Head
        </button>
      </SheetTrigger>
