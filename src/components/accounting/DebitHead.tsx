...
import type { DebitHead, CreditHead } from "@/types/accounting.types";
...
export default function DebitHead() {
  ...
  const { data, isFetching, isError } = useGetExpenseHeadsQuery();

  const debitHeads: CreditHead[] = data?.data || [];

  console.log("Debit Heads", data);

  // Stats
  const totalHeads = debitHeads.length || 0;

  const activeHeads = debitHeads.filter((h: CreditHead) => h.is_active).length;
  const inactiveHeads = debitHeads.filter((h: CreditHead) => !h.is_active).length;
  ...
  return (
    <div>
      <PageHeader
        title="List of Debit Heads"
        actions={<AddDebitHeadForm />}
      />
