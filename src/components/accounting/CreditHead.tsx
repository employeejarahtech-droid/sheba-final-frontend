...
export default function CreditHead() {
  ...
  const { data, isFetching, isError } = useGetIncomeHeadsQuery();
  const creditHeads: CreditHead[] = data?.data || [];

  console.log("creditHeads", data);

  // Stats
  const totalHeads = creditHeads.length || 0;
  // Calculating active/inactive from the current data
  const activeHeadsCurrent = creditHeads.filter((h: CreditHead) => h.is_active).length || 0;
  const inactiveHeadsCurrent = creditHeads.filter((h: CreditHead) => !h.is_active).length || 0;
  ...
  return (
    <div>
      <PageHeader
        title="List of Credit Heads"
        actions={<AddCreditHeadForm />}
      />
