...
function AllUltrasonogramReports() {
  ...
  const { data: ultrasonogramAllReports, isFetching } = useQuery({
    queryKey: ["ultrasonogram-all", page, limit, search, statusFilter, from, to, orderBy],
    queryFn: async () => {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/ultrasonogram-all?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}${statusParam}${fromParam}${toParam}&orderBy=${encodeURIComponent(orderBy)}`,
        {
