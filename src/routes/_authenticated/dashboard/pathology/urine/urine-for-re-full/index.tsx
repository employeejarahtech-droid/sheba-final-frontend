...
function UrineForReFull() {
  ...
  const { data: urineReData, isFetching } = useQuery({
    queryKey: ["urine-re", page, limit, search, statusFilter, from, to],
    queryFn: async () => {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/urine-re?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}${statusParam}${fromParam}${toParam}`,
        {
