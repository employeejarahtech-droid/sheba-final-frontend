...
function UrineForAlbumin() {
  ...
  useEffect(() => {
    const handleExpandClick = async (e: Event) => {
      ...
      let htmlContent = `
        <!-- Header -->
        <div class="bg-gradient-to-r from-sky-600 to-blue-600 text-white px-6 py-5">
          <div class="flex justify-between items-center">
            <div>
              <h2 class="text-xl font-semibold">Urine For Albumin Report</h2>
        ...
        <div class="px-6 py-4 bg-gray-50 flex justify-end gap-3">
          ...
          <button class="edit-urine-albumin-btn inline-flex items-center justify-center rounded-lg text-sm font-medium bg-sky-600 text-white hover:bg-sky-700 h-10 px-5 transition shadow-md" data-row="${btn.dataset.rowData}">
            Edit
          </button>
        </div>
      `;
      ...
      if (resultsContainer) {
        try {
          ...
          if (res.ok) {
            ...
            // Build test results HTML with Albumin result
            let testResultsHTML = `
              <div class="border rounded-xl p-4 hover:shadow-md transition bg-gray-50">
                <div class="flex justify-between items-center mb-3">
                  ...
                  <span class="text-xs px-3 py-1 rounded-full bg-sky-100 text-sky-700 font-medium">
                    Urine Albumin
                  </span>
                </div>
                ...
                <table class="w-full text-sm">
                  ...
                  <tbody>
                    <tr class="border-b">
                      <td class="px-2 py-2">Albumin</td>
                      <td class="px-2 py-2 font-medium">${reportData.albumin || "-"}</td>
  ...
  const stats = [
    { label: "Total Reports", value: data?.data?.meta?.total || 0, icon: FileText, grad: "from-sky-500 to-blue-500" },
    { label: "Albumin Tests", value: items.length || 0, icon: Droplets, grad: "from-blue-500 to-indigo-500" },
    { label: "Recent", value: todayCount, icon: Clock, grad: "from-cyan-500 to-blue-500" },
