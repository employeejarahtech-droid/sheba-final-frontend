...
function HematologyAllReports() {
  ...
  const { data: reportData, isLoading, error } = useQuery({
    queryKey: ["hematology-report", reportId],
    queryFn: async () => {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/hematology-all/${reportId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      if (!res.ok) throw new Error("Failed to fetch hematology report");
      return res.json();
    },
      ...
      <Main>
        <div className="print:hidden flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <Link to="/dashboard/pathology/hematology/all/edit/$id" params={{ id: reportId }}>
        ...
        <HematologyReportDetails invoice={invoice} testName="Hematology Report" paddingTop={paddingTop} fontSize={FONT_SIZE_OPTIONS[fontSize].zoom} showSignature={showSignature} columnLayout={columnLayout} />
      </Main>
    </>
  )
}
