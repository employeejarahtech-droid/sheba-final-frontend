import ReportDetails from '@/features/pathology/custom-tests/components/ReportDetails';
...
function CustomTestReport() {
  ...
  const { data: reportData, isLoading, error } = useQuery({
    queryKey: ["custom-tests-report", reportId],
    queryFn: async () => {
    ...
    <>
      ...
      <Main>
        <div className="print:hidden flex items-center justify-between gap-4">
          <Link to="/dashboard/pathology/custom-tests">
          ...
          <div className="flex items-center gap-4">
            <Link to="/dashboard/pathology/custom-tests/edit/$id" params={{ id: reportId }}>
        ...
        <ReportDetails invoice={invoice} testName="Custom Test Report" paddingTop={paddingTop} fontSize={FONT_SIZE_OPTIONS[fontSize].zoom} columnLayout={columnLayout} />
      </Main>
    </>
  )
}
