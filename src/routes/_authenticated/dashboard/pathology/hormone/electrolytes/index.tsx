...
function Electrolytes() {
    const can = useCan();
    const canEdit = can('pathology.hormone.electrolytes.edit');
  ...
  const { data: electrolytesReports, isFetching } = useQuery({
    queryKey: ["electrolytes", page, limit, search, statusFilter, from, to, orderBy],
    queryFn: async () => {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/electrolytes?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}${statusParam}${fromParam}${toParam}&orderBy=${encodeURIComponent(orderBy)}`,
        {
      ...
      render: (_data: any, _type: string, row: ReportsItem) => {
        return `
          <div class="flex flex-nowrap items-center gap-2">
            ${canEdit ? `<button onclick="window.editElectrolytes(${row.id}, ${row.invoice_id})" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded text-xs font-semibold shadow transition-colors">
            ...
            <a href="/dashboard/pathology/hormone/electrolytes/report/${row.id}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-600 hover:bg-slate-700 text-white rounded text-xs font-semibold shadow transition-colors">
    ...
    const handleExpandClick = async (e: Event) => {
      ...
      let htmlContent = `
        ...
        <div class="px-6 py-4 bg-gray-50 flex justify-end gap-3">
          ...
          ${canEdit ? `<button onclick="window.editElectrolytes(${id}, ${invoiceId})"
  ...
  const stats = [
    { label: "Total Reports", value: electrolytesReports?.data?.meta?.total || 0, icon: FileText, grad: "from-fuchsia-500 to-pink-500" },
    { label: "Current Page", value: items.length, icon: FlaskConical, grad: "from-pink-500 to-rose-500" },
    { label: "Recent Today", value: todayCount, icon: Clock, grad: "from-purple-500 to-fuchsia-500" },
    ...
    <>
      ...
      <main>
        <div className="space-y-3">
          ...
          <DataTable
            hideExport columns={columns} data={items} meta={meta} onPageChange={setPage} search={search} onSearchChange={setSearch} tableTitle="Serum Electrolytes" onLimitChange={setLimit} isLoading={isFetching}
            filterSlot={
              <>
