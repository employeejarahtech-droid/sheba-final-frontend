...
function AllHormones() {
  ...
  const { data: hormoneAllReports, isFetching } = useQuery({
    queryKey: ["hormon-all", page, limit, search, statusFilter, from, to],
    queryFn: async () => {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/hormon-all?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}${statusParam}${fromParam}${toParam}`,
        {
      ...
      render: (_data: any, _type: string, row: ReportsItem) => {
        return `
          <div class="flex flex-nowrap items-center gap-2">
            ...
            <a href="/dashboard/pathology/hormone/all/report/${row.ReciptID}" target="_blank" rel="noopener noreferrer" title="Print" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-600 hover:bg-slate-700 text-white rounded text-xs font-semibold shadow transition-colors">
