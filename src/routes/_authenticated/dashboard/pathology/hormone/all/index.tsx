...
function AllHormones() {
  ...
  const { data: hormoneAllReports, isFetching } = useQuery({
    queryKey: ["hormon-all", page, limit, search, statusFilter, from, to, orderBy],
    queryFn: async () => {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/hormon-all?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}${statusParam}${fromParam}${toParam}&orderBy=${encodeURIComponent(orderBy)}`,
        {
      ...
      render: (_data: any, _type: string, row: ReportsItem) => {
        return `
          <div class="flex flex-nowrap items-center gap-2">
            <a href="/dashboard/pathology/hormone/all/edit/${row.ReciptID}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded text-xs font-semibold shadow transition-colors">
            ...
            <a href="/dashboard/pathology/hormone/all/report/${row.ReciptID}" title="Print" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-600 hover:bg-slate-700 text-white rounded text-xs font-semibold shadow transition-colors">
  ...
  const uniquePatients = new Set(items.map((i: any) => i.PatientId).filter(Boolean)).size;

  const stats = [
    { label: "Total Reports", value: hormoneAllReports?.data?.meta?.total || 0, icon: FileText, grad: "from-violet-500 to-purple-500" },
    { label: "Current Page", value: items.length, icon: FlaskConical, grad: "from-purple-500 to-indigo-500" },
    { label: "Recent Today", value: todayCount, icon: Clock, grad: "from-fuchsia-500 to-purple-500" },
    { label: "Unique Patients", value: uniquePatients, icon: Users, grad: "from-indigo-500 to-blue-500" },
  ];
    ...
    <>
      ...
      <main>
        <div className="space-y-3">
          ...
          <DataTable
            hideExport columns={columns} data={items} meta={meta} onPageChange={setPage} search={search} onSearchChange={setSearch} tableTitle="All Reports (Hormones)" onLimitChange={setLimit} isLoading={isFetching}
            filterSlot={
              <>
