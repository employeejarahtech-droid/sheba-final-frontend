...
import { ArrowLeft, User, FlaskConical, Activity } from 'lucide-react';
import { toast } from 'sonner';

export const Route = createFileRoute(
  '/_authenticated/dashboard/pathology/hormone/all/edit/$id/',
)({
  component: EditReportHormone,
})
...
type InvoiceData = {
  ...
  hormon_all_info: LabTest[]
}



function EditReportHormone() {
  ...
  const { data: invoiceData, isLoading } = useQuery({
    queryKey: ["hormon-invoice", id],
    queryFn: async (): Promise<InvoiceData> => {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/hormon-all/${id}`,
        {
      ...
      if (!res.ok) {
        throw new Error(`Failed to fetch Hormon data (${res.status}: ${res.statusText})`);
      }
  ...
  useEffect(() => {
    if (invoiceData?.hormon_all_info) {
  ...
  const updateMutation = useMutation({
    mutationFn: async (hormonId: number) => {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/hormon-all/${hormonId}`,
        {
          method: 'PUT',
          headers: {
      ...
      if (!res.ok) throw new Error("Failed to update Hormon record");
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["hormon-all", id] });
      queryClient.invalidateQueries({ queryKey: ["outdoor-invoice", id] });
      queryClient.invalidateQueries({ queryKey: ["hormon-all"] });
  ...
  const handleSaveAll = () => {
    ...
    Object.keys(testResults).forEach(hormonId => {
      updateMutation.mutate(Number(hormonId));
    });
  };

  const handlePrint = () => {
    router.navigate({
      to: '/dashboard/pathology/hormone/all/report/$reportId',
      params: { reportId: id },
    });
  };
  ...
  const tests = invoiceData?.hormon_all_info || [];
          ...
          <div className="flex flex-wrap justify-between items-start gap-4 mb-4">
            <div className="flex items-center gap-4">
              <Button
                variant="ghost"
                size="icon"
                onClick={() => router.navigate({ to: '/dashboard/pathology/hormone/all' })}
              >
              ...
              <div>
                ...
                <p className="text-muted-foreground text-sm">Review and update hormone laboratory findings</p>
              </div>
            </div>
          </div>
          ...
          <Card className="overflow-hidden transition-all duration-300 gap-0 shadow-none p-0 border">
            <CardHeader className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950/30 dark:to-indigo-950/30 border-b py-1.5 px-4 gap-0">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <CardTitle className="text-lg font-bold">Hormone Test Results</CardTitle>
            ...
            <CardContent className="p-4">
              <form onSubmit={(e) => { e.preventDefault(); handleSaveAll(); }} className="space-y-6">
                ...
                <div className="flex justify-between gap-3 pt-4">
                  ...
                  <Button type="button" variant="info" className="flex-1" onClick={() => router.navigate({ to: '/dashboard/pathology/hormone/all' })}>
                    View
                  </Button>
                </div>
              </form>
...
export default EditReportHormone
