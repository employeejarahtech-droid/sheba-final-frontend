...
function Sputum() {
    const can = useCan();
    const canEdit = can('pathology.hormone.sputum.edit');
  ...
  const { data: sputumReports, isFetching } = useQuery({
    queryKey: ["sputum", page, limit, search, statusFilter, from, to, orderBy],
    queryFn: async () => {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/sputum?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}${statusParam}${fromParam}${toParam}&orderBy=${encodeURIComponent(orderBy)}`,
        {
      ...
      render: (_data: any, _type: string, row: ReportsItem) => {
        return `
          <div class="flex flex-nowrap items-center gap-2">
            ${canEdit ? `<button onclick="window.editSputum(${row.id}, ${row.invoice_id})" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded text-xs font-semibold shadow transition-colors">
    ...
    const handleExpandClick = async (e: Event) => {
      ...
      let htmlContent = `
        ...
        <div class="px-6 py-4 bg-gray-50 flex justify-end gap-3">
          ...
          ${canEdit ? `<button onclick="window.editSputum(${id}, ${invoiceId})"
  ...
  const stats = [
    { label: "Total Reports", value: sputumReports?.data?.meta?.total || 0, icon: FileText, grad: "from-emerald-500 to-teal-500" },
    { label: "Current Page", value: items.length, icon: FlaskConical, grad: "from-teal-500 to-emerald-500" },
    { label: "Recent Today", value: todayCount, icon: Clock, grad: "from-cyan-500 to-teal-500" },
    ...
    <>
      ...
      <main>
        <div className="space-y-3">
          ...
          <DataTable
            hideExport columns={columns} data={items} meta={meta} onPageChange={setPage} search={search} onSearchChange={setSearch} tableTitle="Sputum Tests" onLimitChange={setLimit} isLoading={isFetching}
            filterSlot={
              <>
