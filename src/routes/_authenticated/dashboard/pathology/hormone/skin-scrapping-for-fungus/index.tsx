...
function SkinScrappingForFungus() {
  ...
  const { data: skinScrapingReports, isFetching } = useQuery({
    queryKey: ["skin-scraping", page, limit, search, statusFilter, from, to],
    queryFn: async () => {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/skin-scraping?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}${statusParam}${fromParam}${toParam}`,
        {
