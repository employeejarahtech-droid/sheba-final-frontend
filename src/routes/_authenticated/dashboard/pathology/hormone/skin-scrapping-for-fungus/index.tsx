...
function SkinScrappingForFungus() {
    const can = useCan();
    const canEdit = can('pathology.hormone.skin-scrapping-for-fungus.edit');
  ...
  const { data: skinScrapingReports, isFetching } = useQuery({
    queryKey: ["skin-scraping", page, limit, search, statusFilter, from, to, orderBy],
    queryFn: async () => {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/skin-scraping?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}${statusParam}${fromParam}${toParam}&orderBy=${encodeURIComponent(orderBy)}`,
        {
      ...
      render: (_data: any, _type: string, row: ReportsItem) => {
        return `
          <div class="flex flex-nowrap items-center gap-2">
            ${canEdit ? `<button onclick="window.editSkinScraping(${row.id}, ${row.invoice_id})" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded text-xs font-semibold shadow transition-colors">
            ...
            <a href="/dashboard/pathology/hormone/skin-scrapping-for-fungus/report/${row.id}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-600 hover:bg-slate-700 text-white rounded text-xs font-semibold shadow transition-colors">
    ...
    const handleExpandClick = async (e: Event) => {
      ...
      let htmlContent = `
        ...
        <div class="px-6 py-4 bg-gray-50 flex justify-end gap-3">
          ...
          ${canEdit ? `<button onclick="window.editSkinScraping(${id}, ${invoiceId})"
    ...
    <>
      ...
      <main>
        <div className="space-y-3">
          ...
          <DataTable
            hideExport columns={columns} data={items} meta={meta} onPageChange={setPage} search={search} onSearchChange={setSearch} tableTitle="Skin Scrapping For Fungus" onLimitChange={setLimit} isLoading={isFetching}
            filterSlot={
              <>
