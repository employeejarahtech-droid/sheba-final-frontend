...
function Semen() {
    const can = useCan();
    const canEdit = can('pathology.hormone.semen.edit');
  ...
  const { data: semenReports, isFetching } = useQuery({
    queryKey: ["semen", page, limit, search, statusFilter, from, to, orderBy],
    queryFn: async () => {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/semen?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}${statusParam}${fromParam}${toParam}&orderBy=${encodeURIComponent(orderBy)}`,
        {
      ...
      render: (_data: any, _type: string, row: ReportsItem) => {
        return `
          <div class="flex flex-nowrap items-center gap-2">
            ${canEdit ? `<a href="/dashboard/pathology/hormone/semen/edit/${row.id}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded text-xs font-semibold shadow transition-colors">
            ...
            <a href="/dashboard/pathology/hormone/semen/report/${row.id}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-600 hover:bg-slate-700 text-white rounded text-xs font-semibold shadow transition-colors">
    ...
    const handleExpandClick = async (e: Event) => {
      ...
      let htmlContent = `
        ...
        <div class="px-6 py-4 bg-gray-50 flex justify-end gap-3">
          ...
          ${canEdit ? `<a href="/dashboard/pathology/hormone/semen/edit/${id}"
  ...
  const stats = [
    { label: "Total Reports", value: semenReports?.data?.meta?.total || 0, icon: FileText, grad: "from-indigo-500 to-violet-500" },
    { label: "Current Page", value: items.length, icon: FlaskConical, grad: "from-violet-500 to-purple-500" },
    { label: "Recent Today", value: todayCount, icon: Clock, grad: "from-blue-500 to-indigo-500" },
    { label: "Unique Patients", value: uniquePatients, icon: Users, grad: "from-purple-500 to-pink-500" },
  ];
    ...
    <>
      ...
      <main>
        <div className="space-y-3">
          ...
          <DataTable
            hideExport columns={columns} data={items} meta={meta} onPageChange={setPage} search={search} onSearchChange={setSearch} tableTitle="Semen Analysis" onLimitChange={setLimit} isLoading={isFetching}
            filterSlot={
              <>
