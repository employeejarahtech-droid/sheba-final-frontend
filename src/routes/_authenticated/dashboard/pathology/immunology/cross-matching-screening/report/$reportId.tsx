import CrossMatchingScreeningReportDetails from '@/features/pathology/immunology/CrossMatchingScreeningReportDetails'
import { createFileRoute, Link } from '@tanstack/react-router'
import { useQuery } from '@tanstack/react-query';
import { getCookie } from '@/lib/cookies';
import { ArrowLeft, Loader2, Printer, Settings2 } from "lucide-react";
...
export const Route = createFileRoute(
    ...
    component: CrossMatchingScreeningReport,
})

function CrossMatchingScreeningReport() {
    ...
    // Fetch cross matching & screening test data
    const { data: cmsData, isLoading: isLoadingCms, error: cmsError } = useQuery({
    ...
    const { data: invoiceData, isLoading: isLoadingInvoice } = useQuery({
        queryKey: ['outdoor-invoice', cmsData?.invoice_id],
        queryFn: async () => {
            const res = await fetch(
                `${import.meta.env.VITE_API_URL}/api/outdoor-invoice/${cmsData.invoice_id}`,
                {
                    method: 'GET',
                    headers: { Authorization: `Bearer ${token}` },
                }
        ...
        enabled: !!token && !!cmsData?.invoice_id,
    });

    if (isLoadingCms || isLoadingInvoice) {
        return (
    ...
    if (cmsError) {
        return (
            <div className="flex items-center justify-center min-h-screen">
                <p className="text-red-600">Error loading cross matching &amp; screening report: {(cmsError as Error).message}</p>
            </div>
        );
    }
        ...
        <>
            ...
            <Main>
                <div className="print:hidden flex items-center justify-between gap-4">
                    <Link to="/dashboard/pathology/immunology/cross-matching-screening">
                        <Button variant="outline" size="sm">
                            <ArrowLeft className="mr-2 h-4 w-4" />
                            Back to Cross Matching &amp; Screening
                        </Button>
                    </Link>
                ...
                <CrossMatchingScreeningReportDetails cmsData={cmsData} invoiceData={invoiceData} paddingTop={paddingTop} fontSize={FONT_SIZE_OPTIONS[fontSize].zoom} showSignature={showSignature} />
            </Main>
        </>
    )
}
