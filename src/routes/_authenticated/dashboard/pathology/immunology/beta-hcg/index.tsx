...
import { FileText, TestTube, Clock, Users, Check, Filter } from 'lucide-react';
...
function BetaHcg() {
  ...
  const { data, isFetching } = useQuery({
    queryKey: ["hcg", page, limit, search, statusFilter, from, to, orderBy],
    ...
    queryFn: async () => {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/hcg?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}${statusParam}${fromParam}${toParam}&orderBy=${encodeURIComponent(orderBy)}`,
        {
  ...
  const stats = [
    { label: "Total Reports", value: data?.data?.meta?.total || 0, icon: FileText, grad: "from-amber-500 to-orange-500" },
    { label: "HCG Tests", value: items.length || 0, icon: TestTube, grad: "from-yellow-500 to-amber-500" },
