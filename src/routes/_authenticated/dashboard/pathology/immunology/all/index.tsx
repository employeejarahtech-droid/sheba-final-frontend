...
function ReportsImmunology() {
    const can = useCan();
    const canEdit = can('pathology.immunology.all.edit');
  ...
  const { data, isFetching } = useQuery({
    queryKey: ["immunology-all", page, limit, search, statusFilter, from, to, orderBy],
    queryFn: async () => {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/immunology-all?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}${statusParam}${fromParam}${toParam}&orderBy=${encodeURIComponent(orderBy)}`,
        {
    ...
    const handleExpandClick = async (e: Event) => {
      ...
      let htmlContent = `
        ...
        <div class="px-6 py-4 bg-gray-50 flex justify-end gap-3">
          ...
          ${canEdit ? `<a href="/dashboard/pathology/immunology/all/edit/${reciptId}"
      ...
      render: (_data: any, _type: string, row: ReportsItem) => {
        ...
        return `
          <div class="flex flex-nowrap items-center gap-2">
            ${canEdit ? `<a href="/dashboard/pathology/immunology/all/edit/${id}" title="Edit report"
            ...
            <a href="/dashboard/pathology/immunology/all/report/${id}" title="Print / view report"
  ...
  const uniquePatients = new Set(items.map((i: any) => i.PatientName)).size;

  const stats = [
    { label: "Total Reports", value: data?.data?.meta?.total || 0, icon: FileText, grad: "from-cyan-500 to-blue-500" },
    { label: "Immunology", value: items.length || 0, icon: FlaskConical, grad: "from-blue-500 to-indigo-500" },
