...
import ImmunologyReportDetails from '@/features/pathology/immunology/all-reports/ImmunologyReportDetails'
...
export const Route = createFileRoute(
  ...
  component: ImmunologyAllReports,
})

function ImmunologyAllReports() {
  ...
  const { data: reportData, isLoading, error } = useQuery({
    queryKey: ["immunology-report", reportId],
    queryFn: async () => {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/immunology-all/${reportId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      if (!res.ok) throw new Error("Failed to fetch immunology report");
      return res.json();
    },
    ...
    <>
      ...
      <Main>
        <div className="print:hidden flex items-center justify-between gap-4">
          <Link to="/dashboard/pathology/immunology/all">
          ...
          <div className="flex items-center gap-3">
            <Link to="/dashboard/pathology/immunology/all/edit/$id" params={{ id: reportId }}>
        ...
        <ImmunologyReportDetails invoice={invoice} testName="Immunology Report" paddingTop={paddingTop} fontSize={FONT_SIZE_OPTIONS[fontSize].zoom} showSignature={showSignature} columnLayout={columnLayout} />
      </Main>
    </>
  )
}
