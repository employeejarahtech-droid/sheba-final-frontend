...
function StoolRe() {
  ...
  const { data, isFetching } = useQuery({
    queryKey: ["stool-re", page, limit, search, statusFilter, from, to],
    ...
    queryFn: async () => {
      try {
        ...
        const res = await fetch(
          `${import.meta.env.VITE_API_URL}/api/stool-re?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}${statusParam}${fromParam}${toParam}`,
          {
