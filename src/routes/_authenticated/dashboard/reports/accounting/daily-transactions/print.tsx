...
export const Route = createFileRoute('/_authenticated/dashboard/reports/accounting/daily-transactions/print')({
  validateSearch: searchSchema,
  component: DailyTransactionsPrint,
})
...
function DailyTransactionsPrint() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['print-daily-transactions', search, start_date, end_date],
    queryFn: async () => {
      const params = new URLSearchParams({ page: '1', limit: '9999', search: search ?? '' })
      if (start_date) params.set('start_date', start_date)
      if (end_date) params.set('end_date', end_date)
      const res = await fetch(`${API_URL}/api/accounting/transactions?${params}`, {
  ...
  const stats = useMemo(() => {
    const totalIncome = flatItems
      .filter((r: any) => r.type === 'INCOME' || r.type === 'SALES')
      .reduce((sum: number, r: any) => sum + (Number(r.amount) || 0), 0)
    const totalExpense = flatItems
      .filter((r: any) => r.type === 'EXPENSE' || r.type === 'PURCHASE')
      .reduce((sum: number, r: any) => sum + (Number(r.amount) || 0), 0)
    const cashTransactions = flatItems.filter((r: any) => r.payment_mode === 'CASH').length
    const bankTransactions = flatItems.filter((r: any) => r.payment_mode === 'BANK').length
    const totalFromMeta = data?.data?.meta?.total ?? flatItems.length

    return [
      { label: 'Total Transactions', value: totalFromMeta, color: COLORS[0] },
      { label: 'Total Income', value: totalIncome, color: COLORS[5], isCurrency: true },
      { label: 'Total Expenses', value: totalExpense, color: COLORS[1], isCurrency: true },
      { label: 'Cash Transactions', value: cashTransactions, color: COLORS[2] },
      { label: 'Bank Transactions', value: bankTransactions, color: COLORS[4], isCurrency: true },
    ...
    <div className="max-w-6xl mx-auto w-full p-8 bg-white">
      ...
      <div className="mb-2 flex items-start justify-between gap-6">
        ...
        <div className="w-1/2 text-right">
          <h2 className="text-lg font-bold tracking-widest uppercase">Daily Transactions Report</h2>
          <p className="text-xs text-gray-600 mt-1">Complete record of daily financial transactions</p>
      ...
      <table className="w-full text-xs mt-2">
        <thead>
          <tr className="border-t border-b" style={{ backgroundColor: '#f0f9ff' }}>
            ...
            <th className="px-1.5 py-1 text-left text-[10px] w-[8%]">ID</th>
            <th className="px-1.5 py-1 text-left text-[10px] w-[10%]">Date</th>
            <th className="px-1.5 py-1 text-center text-[10px] w-[10%]">Type</th>
            <th className="px-1.5 py-1 text-right text-[10px] w-[12%]">Amount</th>
            <th className="px-1.5 py-1 text-center text-[10px] w-[10%]">Payment Mode</th>
            <th className="px-1.5 py-1 text-left text-[10px] w-[26%]">Description</th>
            <th className="px-1.5 py-1 text-left text-[10px] w-[20%]">Reference</th>
          </tr>
        </thead>
        <tbody>
          {flatItems.map((item: TransactionItem, idx: number) => {
            const isIncome = item.type === 'INCOME' || item.type === 'SALES';
            return (
              <tr key={item.id || idx} className="border-b border-dashed">
                ...
                <td className="px-1.5 py-1 text-[10px]">{item.date ? safeFormatDate(item.date) : '-'}</td>
                <td className="px-1.5 py-1 text-[10px] text-center capitalize">{item.type || '-'}</td>
                <td className="px-1.5 py-1 text-[10px] text-right font-semibold" style={{ color: isIncome ? '#16a34a' : '#dc2626' }}>{Number(item.amount || 0).toFixed(2)}</td>
                <td className="px-1.5 py-1 text-[10px] text-center capitalize">{item.payment_mode || '-'}</td>
                <td className="px-1.5 py-1 text-[10px] truncate max-w-[200px]" title={item.description}>{item.description || '-'}</td>
                <td className="px-1.5 py-1 text-[10px]">{item.reference_type || '-'} {item.reference_id ? `#${item.reference_id}` : ''}</td>
              </tr>
            );
          })}
          {flatItems.length === 0 && (
            <tr>
              <td colSpan={8} className="px-2 py-4 text-center text-gray-500" style={{ color: '#6b7280 !important' }}>
                <div className="font-semibold text-xs">No transactions found for the selected criteria</div>
