...
export const Route = createFileRoute('/_authenticated/dashboard/reports/accounting/daily-transactions/')({
  component: DailyTransactionsPage,
})

function DailyTransactionsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['daily-transactions', page, limit, search, from, to],
    queryFn: async () => {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/accounting/transactions?${params}`, {
  ...
  const stats = useMemo(() => {
    const totalIncome = items
      .filter((i) => i.type === 'INCOME' || i.type === 'SALES')
      .reduce((sum, i) => sum + (Number(i.amount) || 0), 0)
    const totalExpense = items
      .filter((i) => i.type === 'EXPENSE' || i.type === 'PURCHASE')
      .reduce((sum, i) => sum + (Number(i.amount) || 0), 0)
    const cashTransactions = items.filter((i) => i.payment_mode === 'CASH').length
    const bankTransactions = items.filter((i) => i.payment_mode === 'BANK').length

    return [
      { label: 'Total Transactions', value: meta.total, icon: Hash, grad: 'from-green-500 to-green-600' },
      { label: 'Total Income', value: totalIncome, icon: TrendingUp, grad: 'from-blue-500 to-blue-600', isCurrency: true },
      { label: 'Total Expenses', value: totalExpense, icon: TrendingDown, grad: 'from-orange-500 to-orange-600', isCurrency: true },
      { label: 'This Page', value: items.length, icon: Wallet, grad: 'from-teal-500 to-teal-600' },
      { label: 'Cash Transactions', value: cashTransactions, icon: DollarSign, grad: 'from-pink-500 to-pink-600' },
      { label: 'Bank Transactions', value: bankTransactions, icon: Calendar, grad: 'from-yellow-500 to-yellow-600' },
    ]
  }, [items, meta])
  ...
  useEffect(() => {
    const handleExpandClick = async (e: Event) => {
      ...
      details.innerHTML = `
        ...
        <div class="p-6">
          <div id="transaction-details-${id}" class="text-gray-500 text-sm">
      ...
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/api/accounting/transactions/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
          ...
          <div class="space-y-6">
            <div class="grid grid-cols-2 gap-x-8 gap-y-4 text-sm border-b pb-6">
              ...
              <div>
                ...
                <p class="font-semibold capitalize">${transaction.type || '-'}</p>
              </div>
              <div>
                <p class="text-gray-500">Amount</p>
                <p class="font-semibold text-gray-800">${Number(transaction.amount || 0).toFixed(2)}</p>
              </div>
              <div>
                <p class="text-gray-500">Payment Mode</p>
                <p class="font-semibold capitalize">${transaction.payment_mode || '-'}</p>
              </div>
              <div>
                <p class="text-gray-500">Date</p>
                <p class="font-semibold text-gray-800">${transaction.date ? formatDate(new Date(transaction.date)) : '-'}</p>
              </div>
              <div>
                <p class="text-gray-500">Reference</p>
                <p class="font-semibold text-gray-800">${transaction.reference_type || '-'} ${transaction.reference_id ? `#${transaction.reference_id}` : ''}</p>
              </div>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t">
              <a href="/dashboard/accounting/transactions/${id}/print"
        ...
        const container = document.getElementById(`transaction-details-${id}`);
        if (container) {
  ...
  const columns = [
    {
      ...
      title: "ID",
      orderable: true,
      render: (data: any, _type: string, row: TransactionItem) => {
        return `
          <div class="flex items-center gap-2">
            ...
            <span class="font-mono text-xs text-purple-600 bg-purple-50 px-2 py-1 rounded">#${data || '-'}</span>
          </div>
        `;
      },
    },
    ...
    },
    {
      data: "amount",
      title: "Amount",
      render: (data: number | null, type: string, row: TransactionItem) => {
        const amount = Number(data || 0).toFixed(2)
        const isIncome = row.type === 'INCOME' || row.type === 'SALES'
        const colorClass = isIncome ? 'text-green-600' : 'text-red-600'
        return `<span class="font-semibold ${colorClass}">${amount}</span>`
      },
    },
    {
      data: "payment_mode",
      ...
      render: (data: string | null) => {
        ...
        const colorClass = colorMap[mode] || 'bg-gray-100 text-gray-700'
        return `<span class="px-2 py-0.5 rounded-full text-xs font-medium capitalize ${colorClass}">${mode || '-'}</span>`
      },
    },
    {
      data: "description",
    ...
    },
    {
      data: "reference_type",
      title: "Reference",
      render: (data: string | null, type: string, row: TransactionItem) => {
        const refType = data || '-'
        const refId = row.reference_id ? `#${row.reference_id}` : ''
        return `<span class="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded">${refType} ${refId}</span>`
      },
    },
    {
      data: null,
      title: "Actions",
      orderable: false,
      render: (_data: any, _type: string, row: TransactionItem) => {
        const id = row.id;
        return `<div class="flex gap-2">
          <a href="/dashboard/accounting/transactions/${id}/print" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded text-xs font-semibold shadow transition-colors">
  ...
  return (
    <>
      <AppHeader
        ...
        description="Complete record of daily financial transactions with filtering, search, and expandable details"
        fixed
      />
      ...
      <main className="">
              ...
              <Card key={index} className="overflow-hidden transition-all duration-300 gap-0 shadow-none p-0 border">
                ...
                <CardContent className="p-4">
                  <p className="text-2xl font-bold">
                    {stat.isCurrency ? stat.value.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : stat.value.toLocaleString()}
                  </p>
                </CardContent>
              </Card>
            )
        ...
        <DataTable
          tableTitle="Daily Transactions List"
          columns={columns}
          data={items}
          meta={meta}
          ...
          filterSlot={
            <div className="flex items-center gap-1.5">
              ...
              <Link
                to="/dashboard/reports/accounting/daily-transactions/print"
                search={{
