...
export const Route = createFileRoute('/_authenticated/dashboard/reports/patient/admission-register/print')({
  ...
  component: AdmissionRegisterPrint,
})
...
function AdmissionRegisterPrint() {
  ...
  const stats = useMemo(() => {
    ...
    const maleCount = flatItems.filter((r: any) => r.sex === 'male').length
    const femaleCount = flatItems.filter((r: any) => r.sex === 'female').length
    const totalFromMeta = data?.data?.meta?.total ?? flatItems.length

    return [
      { label: 'Total Admissions', value: totalFromMeta },
      { label: 'Active Patients', value: activeCount },
      { label: 'Discharged', value: dischargedCount },
      { label: 'Male Patients', value: maleCount },
      { label: 'Female Patients', value: femaleCount },
  ...
  console.log('Print data state:', { isLoading, flatItemsLength: flatItems.length, items, data })

  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-screen gap-3">
        <Loader2 className="h-8 w-8 animate-spin" />
        <span className="text-gray-600">Loading admission data...</span>
      </div>
    )
  }
      ...
      <table className="w-full text-xs mt-2">
        <thead>
          <tr className="border-t border-b" style={{ backgroundColor: '#f0f9ff' }}>
            ...
            <th className="px-1.5 py-1 text-left text-[10px] w-[9%]">Adm. No</th>
            <th className="px-1.5 py-1 text-left text-[10px] w-[14%]">Patient Name</th>
            <th className="px-1.5 py-1 text-center text-[10px] w-[5%]">Age</th>
            <th className="px-1.5 py-1 text-center text-[10px] w-[6%]">Gender</th>
            <th className="px-1.5 py-1 text-left text-[10px] w-[10%]">Phone</th>
            <th className="px-1.5 py-1 text-left text-[10px] w-[9%]">Admission Date</th>
            <th className="px-1.5 py-1 text-left text-[10px] w-[9%]">Ward/Bed</th>
            <th className="px-1.5 py-1 text-left text-[10px] w-[12%]">Doctor</th>
            <th className="px-1.5 py-1 text-left text-[10px] w-[10%]">Department</th>
            <th className="px-1.5 py-1 text-left text-[10px] w-[13%]">Discharge Date</th>
          </tr>
        </thead>
        <tbody>
          {flatItems.map((item: AdmissionItem, idx: number) => (
            <tr key={item.id || idx} className="border-b border-dashed">
              ...
              <td className="px-1.5 py-1 text-[10px]">
                {item.bedCabin?.ward && item.bedCabin?.code ? `${item.bedCabin.ward}/${item.bedCabin.code}` : (item.bedCabin?.ward || item.bedCabin?.code || item.ward_name || item.bed_name || '-')}
              </td>
          ...
          {flatItems.length === 0 && (
            <tr>
              <td colSpan={11} className="px-2 py-4 text-center text-gray-500" style={{ color: '#6b7280 !important' }}>
                <div className="font-semibold text-xs">No admission records found for the selected criteria</div>
