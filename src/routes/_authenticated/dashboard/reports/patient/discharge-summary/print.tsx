...
function DischargeSummaryPrint() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const params = new URLSearchParams({ page: '1', limit: '9999', search: search ?? '', status: 'discharged' })
  ...
  const stats = useMemo(() => {
    const paidCount = flatItems.filter((r: any) => Number(r.due_amount || 0) <= 0).length
    const dueCount = flatItems.filter((r: any) => Number(r.due_amount || 0) > 0).length
    ...
    return [
      { label: 'Total Discharged', value: totalFromMeta },
      { label: 'Fully Paid', value: paidCount },
      { label: 'With Due Amount', value: dueCount },
  ...
  return (
    <div className="invoice-print-area max-w-6xl mx-auto w-full p-8 bg-white mt-10 print:mt-0 shadow-sm print:shadow-none border border-slate-100 print:border-none rounded-lg print:rounded-none">
      ...
      <p className="text-center text-xs text-gray-600 mb-2">Discharged patient summary with billing information</p>
      ...
      <table className="w-full text-xs mt-2">
        <thead>
          <tr className="border-t border-b bg-row-blue">
            ...
            <th className="px-1.5 py-1 text-left text-[10px] w-[8%]">Adm. No</th>
            <th className="px-1.5 py-1 text-left text-[10px] w-[13%]">Patient Name</th>
            ...
            <th className="px-1.5 py-1 text-right text-[10px] w-[9%]">Total Bill</th>
            <th className="px-1.5 py-1 text-right text-[10px] w-[9%]">Amount Paid</th>
            <th className="px-1.5 py-1 text-right text-[10px] w-[9%]">Due Amount</th>
        ...
        <tbody>
          {flatItems.map((item: DischargeItem, idx: number) => (
            <tr key={item.id || idx} className="border-b border-dashed">
              ...
              <td className="px-1.5 py-1 text-[10px] text-right font-semibold">৳{Number(item.total_bill || 0).toLocaleString()}</td>
              <td className="px-1.5 py-1 text-[10px] text-right font-semibold text-green-600">৳{Number(item.amount_paid || 0).toLocaleString()}</td>
              <td className={`px-1.5 py-1 text-[10px] text-right font-semibold ${Number(item.due_amount || 0) > 0 ? 'text-red-600' : 'text-gray-600'}`}>৳{Number(item.due_amount || 0).toLocaleString()}</td>
