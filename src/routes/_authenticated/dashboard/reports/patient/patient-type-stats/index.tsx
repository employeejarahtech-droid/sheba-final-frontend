...
import { Users, Building2, UserCheck, AlertCircle, Hash, Printer, FileText, Calendar } from 'lucide-react'
...
export const Route = createFileRoute('/_authenticated/dashboard/reports/patient/patient-type-stats/')({
  component: PatientTypeStatsPage,
})

function PatientTypeStatsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['patient-type-stats', page, limit, search, from, to],
    queryFn: async () => {
  ...
  const items: PatientTypeItem[] = data?.data?.items || []
  const meta: Meta = data?.data?.meta || { total: 0, page: 1, limit: 10, totalPages: 1 }

  // Calculate statistics
  const stats = useMemo(() => {
    const indoorCount = items.filter((i) => String(i.patient_type).toLowerCase().includes('indoor')).length
    const outdoorCount = items.filter((i) => String(i.patient_type).toLowerCase().includes('outdoor')).length
    const emergencyCount = items.filter((i) => String(i.patient_type).toLowerCase().includes('emergency')).length

    return [
      { label: 'Total Patients', value: meta.total, icon: Users, grad: 'from-green-500 to-green-600' },
      { label: 'Indoor', value: indoorCount, icon: Building2, grad: 'from-blue-500 to-blue-600' },
      { label: 'Outdoor', value: outdoorCount, icon: UserCheck, grad: 'from-orange-500 to-orange-600' },
      { label: 'Emergency', value: emergencyCount, icon: AlertCircle, grad: 'from-red-500 to-red-600' },
      { label: 'This Page', value: items.length, icon: Hash, grad: 'from-teal-500 to-teal-600' },
      { label: 'Date Range', value: from || to ? 'Filtered' : 'All', icon: Calendar, grad: 'from-purple-500 to-purple-600' },
    ]
  }, [items, meta, from, to])
    ...
    const handleExpandClick = async (e: Event) => {
      ...
      details.innerHTML = `
        <div class="bg-gradient-to-r from-purple-600 to-purple-500 text-white px-6 py-4">
          ...
          <p class="text-sm opacity-90">Patient ID: ${id}</p>
        </div>
        <div class="p-6">
          <div id="patient-details-${id}" class="text-gray-500 text-sm">
      ...
      try {
          ...
          <div class="space-y-6">
            <div class="grid grid-cols-2 gap-x-8 gap-y-4 text-sm border-b pb-6">
              <div>
                ...
                <p class="font-semibold text-gray-800">${patient.patient_name || '-'}</p>
              </div>
              <div>
                <p class="text-gray-500">Patient Type</p>
                <p class="font-semibold capitalize">${patient.patient_type || '-'}</p>
              </div>
              <div>
                <p class="text-gray-500">Age / Phone</p>
                <p class="font-semibold text-gray-800">${patient.age ? `${patient.age} yrs` : '-'} / ${patient.phone || '-'}</p>
              </div>
              <div>
                <p class="text-gray-500">Status</p>
                <p class="font-semibold capitalize">${patient.status || '-'}</p>
              </div>
              <div>
                <p class="text-gray-500">Admission Date</p>
                <p class="font-semibold text-gray-800">${patient.admission_date ? formatDate(new Date(patient.admission_date)) : '-'}</p>
              </div>
              <div>
                <p class="text-gray-500">Doctor</p>
                <p class="font-semibold text-gray-800">${patient.doctor_name || '-'}</p>
              </div>
              <div class="col-span-2">
                <p class="text-gray-500">Department</p>
                <p class="font-semibold text-gray-800">${patient.department || '-'}</p>
              </div>
            </div>

            <div class="flex justify-end gap-3 pt-4 border-t">
              <a href="/dashboard/reports/patient/patient-type-stats/print"
        ...
        const container = document.getElementById(`patient-details-${id}`);
        if (container) {
  ...
  const columns = [
    {
      ...
      title: "ID",
      orderable: true,
      render: (data: any, _type: string, row: PatientTypeItem) => {
        return `
    ...
    },
    {
      data: "patient_type",
      title: "Type",
      render: (data: string | null) => typeBadge(data),
    },
    {
      data: "admission_date",
    ...
    },
    {
      ...
      render: (data: string | null) => {
        ...
        const colorMap: Record<string, { color: string; label: string }> = {
          ...
          admitted: { color: 'bg-green-100 text-green-700 border-green-200', label: 'Admitted' },
          discharged: { color: 'bg-gray-100 text-gray-700 border-gray-200', label: 'Discharged' },
        }
    ...
    },
    {
      ...
      render: (_data: any, _type: string, row: PatientTypeItem) => {
        ...
        return `<div class="flex gap-2">
          <a href="/dashboard/reports/patient/patient-type-stats/print?search=${search}&start_date=${from}&end_date=${to}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded text-xs font-semibold shadow transition-colors">
  ...
  return (
    <>
      <AppHeader
        ...
        description="Detailed breakdown of patients by type (indoor, outdoor, emergency) with comprehensive filtering"
        fixed
      />
      ...
      <main className="">
        ...
        <DataTable
          tableTitle="Patient Type Statistics List"
          columns={columns}
          data={items}
          meta={meta}
          ...
          filterSlot={
            <div className="flex items-center gap-1.5">
              ...
              <Link
                to="/dashboard/reports/patient/patient-type-stats/print"
                search={{
