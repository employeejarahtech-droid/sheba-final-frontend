...
import { UserCog, Users, DollarSign, TrendingUp, Printer, FileText, Stethoscope } from 'lucide-react'
...
export const Route = createFileRoute('/_authenticated/dashboard/reports/patient/consultant-wise-patients/')({
  component: ConsultantWisePatientsPage,
})

function ConsultantWisePatientsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['consultant-wise-patients', page, limit, search, from, to],
    queryFn: async () => {
  ...
  const items: DoctorItem[] = data?.data?.items || []
  const meta: Meta = data?.data?.meta || { total: 0, page: 1, limit: 10, totalPages: 1 }

  // Calculate statistics
  const stats = useMemo(() => {
    const totalDoctors = items.length
    const totalPatients = items.reduce((sum: number, d: DoctorItem) => sum + (d.patient_count || 0), 0)
    const totalRevenue = items.reduce((sum: number, d: DoctorItem) => sum + (d.total_bill || 0), 0)
    const totalCollected = items.reduce((sum: number, d: DoctorItem) => sum + (d.total_collected || 0), 0)
    ...
    return [
      { label: 'Total Consultants', value: totalDoctors, icon: UserCog, grad: 'from-green-500 to-green-600' },
      ...
      { label: 'Avg per Consultant', value: avgPatients, icon: TrendingUp, grad: 'from-teal-500 to-teal-600' },
  ...
  const columns = [
    {
      ...
      render: (data: string, type: string, row: DoctorItem) => {
        const doctorName = data || row.doctor_name || '-'
        return `
    ...
    },
    {
      ...
      render: (data: number) => {
        return `<span class="font-mono font-semibold text-blue-600">${(data || 0).toLocaleString()}</span>`
      },
    },
    {
      data: "total_bill",
    ...
    },
    {
      ...
      render: (_data: any, _type: string, row: DoctorItem) => {
        const bill = row.total_bill || 0
        const collected = row.total_collected || 0
        const rate = bill > 0 ? ((collected / bill) * 100).toFixed(1) : '0.0'
        const colorClass = Number(rate) >= 80 ? 'text-green-600' : Number(rate) >= 50 ? 'text-yellow-600' : 'text-red-600'
        return `<span class="font-semibold ${colorClass}">${rate}%</span>`
      },
    },
    {
      data: null,
      ...
      render: (_data: any, _type: string, row: DoctorItem) => {
        ...
        return `<div class="flex gap-2">
          <a href="/dashboard/reports/patient/consultant-wise-patients/print?search=${encodeURIComponent(search)}&start_date=${from || ""}&end_date=${to || ""}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded text-xs font-semibold shadow transition-colors">
  ...
  return (
    <>
      <AppHeader
        title="Consultant Wise Patient Report"
        description="Admitted patient distribution and revenue collection by consulting doctor with filtering and statistics"
        fixed
      />
      ...
      <main className="">
        ...
        <DataTable
          tableTitle="Consultant Wise Patient List"
          columns={columns}
          data={items}
          meta={meta}
          ...
          filterSlot={
            <div className="flex items-center gap-1.5">
              ...
              <Link
                to="/dashboard/reports/patient/consultant-wise-patients/print"
                search={{
          ...
          emptyState={
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 font-medium">No consultant records found</p>
