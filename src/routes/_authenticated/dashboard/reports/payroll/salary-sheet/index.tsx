...
import { Users, DollarSign, TrendingDown, Wallet, Printer, FileText, Calendar } from 'lucide-react'
...
export const Route = createFileRoute('/_authenticated/dashboard/reports/payroll/salary-sheet/')({
...
function SalarySheetPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['salary-sheet', page, limit, search, from, to],
    queryFn: async () => {
  ...
  const items: SalaryItem[] = data?.data?.items || []
  ...
  const stats = useMemo(() => {
    ...
    const totalNet = items.reduce((sum, r) => sum + (Number(r.net_salary) || 0), 0)
    const totalDeductions = items.reduce((sum, r) => sum + (Number(r.deductions) || 0), 0)
    const paidCount = items.filter((r) => r.status?.toLowerCase() === 'paid').length
    const pendingCount = items.filter((r) => r.status?.toLowerCase() === 'pending').length

    return [
      { label: 'Total Staff', value: meta.total, icon: Users, grad: 'from-green-500 to-green-600' },
      { label: 'Total Basic', value: `৳${totalBasic.toLocaleString()}`, icon: DollarSign, grad: 'from-orange-500 to-orange-600' },
      { label: 'Total Net Payout', value: `৳${totalNet.toLocaleString()}`, icon: Wallet, grad: 'from-pink-500 to-pink-600' },
      { label: 'Total Deductions', value: `৳${totalDeductions.toLocaleString()}`, icon: TrendingDown, grad: 'from-teal-500 to-teal-600' },
      { label: 'Paid Staff', value: paidCount, icon: Calendar, grad: 'from-yellow-500 to-yellow-600' },
      { label: 'Pending Staff', value: pendingCount, icon: FileText, grad: 'from-blue-500 to-blue-600' },
    ]
  }, [items, meta])
  ...
  const statusBadge = (status: string) => {
    ...
    return (
      <span className={`px-2 py-0.5 rounded text-xs font-medium capitalize ${map[status?.toLowerCase()] ?? 'bg-gray-100 text-gray-600'}`}>
        {status ?? '—'}
      </span>
    )
  }
  ...
  const columns = [
    {
      ...
      title: "#",
      orderable: false,
      render: (_data: any, _type: string, row: SalaryItem, meta: any) => {
        return String((meta.page - 1) * meta.limit + meta.row + 1)
      },
    },
    {
      data: "staff_name",
    ...
    },
    {
      data: "basic_salary",
      title: "Basic Salary",
      render: (data: number) => `৳${Number(data || 0).toLocaleString()}`,
    },
    {
      data: "allowances",
      title: "Allowances",
    ...
    },
    {
      ...
      render: (data: string) => {
        ...
        return `<span class="px-3 py-1 rounded-full text-xs font-semibold capitalize border ${colorClass}">${data || '-'}</span>`
      },
    },
    {
      data: "payment_date",
      ...
      render: (data: string | null) => {
        if (!data) return '<span class="text-gray-400 italic">Not paid</span>'
    ...
    },
    {
      ...
      title: "Actions",
      orderable: false,
      render: (_data: any, _type: string, row: SalaryItem) => {
        const id = row.id
        return `<div class="flex gap-2">
          <a href="/dashboard/reports/payroll/salary-sheet/print" data-id="${id}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded text-xs font-semibold shadow transition-colors">
          ...
          <a href="/dashboard/staff/${id}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-green-600 hover:bg-green-700 text-white rounded text-xs font-semibold shadow transition-colors">
  ...
  return (
    <>
      <AppHeader
        title="Salary Sheet"
        description="Monthly staff salary breakdown with payment status and detailed breakdown"
        fixed
      />
        ...
        <DataTable
          ...
          filterSlot={
            <div className="flex items-center gap-1.5">
              ...
              <Link
                to="/dashboard/reports/payroll/salary-sheet/print"
                search={{
