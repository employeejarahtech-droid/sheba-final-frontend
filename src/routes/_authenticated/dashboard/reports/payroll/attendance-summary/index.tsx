...
import { Users, CheckCircle, XCircle, Clock, Hash, Printer, FileText, Calendar } from 'lucide-react'
...
export const Route = createFileRoute('/_authenticated/dashboard/reports/payroll/attendance-summary/')({
...
function AttendanceSummaryPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['attendance-summary', page, limit, search, from, to],
    queryFn: async () => {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/reports/hr/attendance?${params}`, {
  ...
  const items: AttendanceItem[] = data?.data?.items || []
  ...
  const stats = useMemo(() => {
    ...
    const avgPresent =
      totalStaff > 0
        ? (items.reduce((sum: number, r: any) => sum + (Number(r.present_days) || 0), 0) / totalStaff).toFixed(1)
        : '0'
    const totalAbsent = items.reduce((sum: number, r: any) => sum + (Number(r.absent_days) || 0), 0)
    const totalLate = items.reduce((sum: number, r: any) => sum + (Number(r.late_days) || 0), 0)

    return [
      { label: 'Total Staff', value: totalStaff, icon: Users, grad: 'from-green-500 to-green-600' },
      { label: 'Avg Present Days', value: avgPresent, icon: CheckCircle, grad: 'from-blue-500 to-blue-600' },
      { label: 'Total Absences', value: totalAbsent, icon: XCircle, grad: 'from-orange-500 to-orange-600' },
      { label: 'This Page', value: items.length, icon: Hash, grad: 'from-teal-500 to-teal-600' },
      { label: 'Total Late', value: totalLate, icon: Clock, grad: 'from-pink-500 to-pink-600' },
      { label: 'Records in Period', value: meta.total, icon: Calendar, grad: 'from-yellow-500 to-yellow-600' },
    ]
  }, [items, meta])
    ...
    const handleExpandClick = async (e: Event) => {
      ...
      details.innerHTML = `
        <div class="bg-gradient-to-r from-blue-600 to-blue-500 text-white px-6 py-4">
          ...
          <p class="text-sm opacity-90">Staff ID #${id}</p>
        </div>
        <div class="p-6">
          <div id="attendance-details-${id}" class="text-gray-500 text-sm">
      ...
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/api/reports/hr/attendance/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        ...
        const attendanceDetailsHTML = `
          <div class="space-y-6">
            <div class="grid grid-cols-2 gap-x-8 gap-y-4 text-sm border-b pb-6">
              <div>
                <p class="text-gray-500">Staff Name</p>
                <p class="font-semibold text-gray-800">${[staff.first_name, staff.last_name].filter(Boolean).join(' ') || '-'}</p>
              </div>
              <div>
                <p class="text-gray-500">Employee ID</p>
                <p class="font-semibold text-gray-800">${staff.employee_id || '-'}</p>
              </div>
              <div>
                <p class="text-gray-500">Department</p>
                <p class="font-semibold text-gray-800">${staff.department?.name || '-'}</p>
              </div>
              <div>
                <p class="text-gray-500">Designation</p>
                <p class="font-semibold text-gray-800">${staff.designation || '-'}</p>
              </div>
              <div>
                <p class="text-gray-500">Present Days</p>
                <p class="font-semibold text-green-600">${staff.present_days || 0}</p>
              </div>
              <div>
                <p class="text-gray-500">Absent Days</p>
                <p class="font-semibold text-red-600">${staff.absent_days || 0}</p>
              </div>
              <div>
                <p class="text-gray-500">Late Days</p>
                <p class="font-semibold text-orange-600">${staff.late_days || 0}</p>
              </div>
              <div>
                <p class="text-gray-500">Half Days</p>
                <p class="font-semibold text-blue-600">${staff.half_days || 0}</p>
              </div>
              <div class="col-span-2">
                <p class="text-gray-500">Total Working Days</p>
                <p class="font-semibold text-gray-800">${staff.total_working_days || 0}</p>
              </div>
            </div>

            <div class="flex justify-end gap-3 pt-4 border-t">
              <a href="/dashboard/reports/payroll/attendance-summary/print"
                 data-search="${search || ''}"
        ...
        const container = document.getElementById(`attendance-details-${id}`);
        if (container) {
          container.innerHTML = attendanceDetailsHTML;
        }
      } catch (error) {
  ...
  const columns = [
    {
      ...
      title: "#",
      orderable: true,
      render: (data: any, _type: string, row: AttendanceItem) => {
        return `
          <div class="flex items-center gap-2">
            ...
            <span class="font-mono text-xs text-purple-600 bg-purple-50 px-2 py-1 rounded">${String(data).padStart(4, '0')}</span>
          </div>
        `;
      },
    },
    {
      data: "first_name",
      title: "Staff Name",
      render: (data: string, type: string, row: AttendanceItem) => {
    ...
    },
    {
      data: "present_days",
      title: "Present Days",
      render: (data: number | null) => `<span class="font-semibold text-green-600">${data || 0}</span>`,
    },
    {
      data: "late_days",
      title: "Late Days",
      render: (data: number | null) => `<span class="font-semibold text-orange-600">${data || 0}</span>`,
    },
    {
      data: "absent_days",
      title: "Absent Days",
      render: (data: number | null) => `<span class="font-semibold text-red-600">${data || 0}</span>`,
    },
    {
      data: "half_days",
      title: "Half Days",
      render: (data: number | null) => `<span class="font-semibold text-blue-600">${data || 0}</span>`,
    },
    {
      data: "total_working_days",
      title: "Total Working Days",
      render: (data: number | null) => `<span class="font-semibold text-gray-700">${data || 0}</span>`,
    },
    {
      data: null,
      title: "Attendance %",
      render: (_data: any, _type: string, row: AttendanceItem) => {
        ...
        const percentage = ((present + half * 0.5) / total * 100).toFixed(1)
        const colorClass = percentage >= 90 ? 'text-green-600' : percentage >= 75 ? 'text-yellow-600' : 'text-red-600'
        return `<span class="font-semibold ${colorClass}">${percentage}%</span>`
      },
    },
    {
      data: null,
      ...
      render: (_data: any, _type: string, row: AttendanceItem) => {
        ...
        return `<div class="flex gap-2">
          ...
          <a href="/dashboard/hr/staff/${id}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-green-600 hover:bg-green-700 text-white rounded text-xs font-semibold shadow transition-colors">
  ...
  return (
    <>
      <AppHeader
        ...
        description="Monthly staff attendance report with filtering, search, and expandable details"
        fixed
      />
      ...
      <main className="">
        ...
        <DataTable
          tableTitle="Attendance Summary List"
          columns={columns}
          data={items}
          meta={meta}
          ...
          filterSlot={
            <div className="flex items-center gap-1.5">
              ...
              <Link
                to="/dashboard/reports/payroll/attendance-summary/print"
                search={{
