...
import { CalendarDays, Users, DollarSign, Wallet, Printer, FileText, Hash, TrendingUp } from 'lucide-react'
...
export const Route = createFileRoute('/_authenticated/dashboard/reports/payroll/payroll-summary/')({
...
function PayrollSummaryPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['payroll-summary', page, limit, search, from, to],
    queryFn: async () => {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/reports/hr/payroll?${params}`, {
    ...
    enabled: !!token,
    placeholderData: (prev) => prev ? prev : { data: { items: [], meta: { total: 0, page: 1, limit: 12, totalPages: 0 } } },
  })

  const items: PayrollItem[] = data?.data?.items || []
  const meta: Meta = data?.data?.meta || { total: 0, page: 1, limit: 12, totalPages: 1 }

  // Calculate statistics
  const stats = useMemo(() => {
    const totalStaffPaid = items.reduce((sum, r) => sum + (Number(r.staff_paid) || 0), 0)
    const annualBasic = items.reduce((sum, r) => sum + (Number(r.total_basic) || 0), 0)
    const annualNet = items.reduce((sum, r) => sum + (Number(r.total_net_payout) || 0), 0)
    const avgMonthlyNet = items.length > 0 ? Math.round(annualNet / items.length) : 0

    return [
      { label: 'Total Months', value: meta.total, icon: CalendarDays, grad: 'from-green-500 to-green-600' },
      { label: 'Staff Paid', value: totalStaffPaid, icon: Users, grad: 'from-blue-500 to-blue-600' },
      { label: 'Annual Basic', value: `৳${annualBasic.toLocaleString()}`, icon: DollarSign, grad: 'from-orange-500 to-orange-600' },
      { label: 'Annual Net', value: `৳${annualNet.toLocaleString()}`, icon: Wallet, grad: 'from-teal-500 to-teal-600' },
      { label: 'Avg Monthly', value: `৳${avgMonthlyNet.toLocaleString()}`, icon: TrendingUp, grad: 'from-pink-500 to-pink-600' },
      { label: 'This Page', value: items.length, icon: Hash, grad: 'from-yellow-500 to-yellow-600' },
    ]
  }, [items, meta])
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (data: number) => {
        return `<span class="font-mono text-sm font-semibold">${data?.toLocaleString() || 0}</span>`
      },
    },
    {
      data: "total_basic",
      ...
      className: "text-right",
      render: (data: number) => {
        const value = Number(data || 0).toLocaleString()
        return `<span class="font-mono text-sm text-blue-600">৳${value}</span>`
      },
    },
    {
      ...
      render: (data: number) => {
        const value = Number(data || 0).toLocaleString()
        return `<span class="font-mono text-sm text-green-600">৳${value}</span>`
      },
    },
    {
      ...
      render: (data: number) => {
        const value = Number(data || 0).toLocaleString()
        return `<span class="font-mono text-sm text-red-600">৳${value}</span>`
      },
    },
    {
      data: "total_net_payout",
      title: "Net Payout",
      className: "text-right",
      render: (data: number) => {
        const value = Number(data || 0).toLocaleString()
        return `<span class="font-mono text-sm font-bold text-gray-800 bg-gray-100 px-2 py-1 rounded">৳${value}</span>`
      },
    },
    {
      data: null,
      title: "Actions",
      orderable: false,
      render: (_data: any, _type: string, row: PayrollItem) => {
        const month = row.month;
        return `<div class="flex gap-2">
          <a href="/dashboard/reports/payroll/payroll-summary/print?search=${search || ''}&start_date=${from || ''}&end_date=${to || ''}"
  ...
  return (
    <>
      <AppHeader
        ...
        description="Annual payroll overview by month with filtering and statistics"
        fixed
      />
      ...
      <main className="">
        ...
        <DataTable
          tableTitle="Payroll Summary List"
          columns={columns}
          data={items}
          meta={meta}
          ...
          filterSlot={
            <div className="flex items-center gap-1.5">
              ...
              <Link
                to="/dashboard/reports/payroll/payroll-summary/print"
                search={{
