...
import { Truck, DollarSign, Users, Package, Hash, Printer, FileText, Calendar, ShoppingCart } from 'lucide-react'
...
function SupplierPurchasesPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['supplier-purchases', page, limit, search, from, to],
    queryFn: async () => {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/inventory/purchases?${params}`, {
  ...
  const items: PurchaseItem[] = data?.data?.items || []
  ...
  const stats = useMemo(() => {
    ...
    const uniqueSuppliers = new Set(items.map((i) => i.supplier_name).filter(Boolean)).size
    const completedCount = items.filter((i) => i.status === 'completed' || i.status === 'Completed' || i.status === 'received').length
    const pendingCount = items.filter((i) => i.status !== 'completed' && i.status !== 'Completed' && i.status !== 'received').length
    const avg = items.length > 0 ? totalAmount / items.length : 0

    return [
      { label: 'Total Purchases', value: meta.total, icon: ShoppingCart, grad: 'from-green-500 to-green-600' },
      { label: 'Total Amount (৳)', value: `৳${totalAmount.toLocaleString()}`, icon: DollarSign, grad: 'from-blue-500 to-blue-600' },
      { label: 'Unique Suppliers', value: uniqueSuppliers, icon: Users, grad: 'from-orange-500 to-orange-600' },
      { label: 'Completed', value: completedCount, icon: Package, grad: 'from-teal-500 to-teal-600' },
      { label: 'Pending', value: pendingCount, icon: Truck, grad: 'from-pink-500 to-pink-600' },
      { label: 'Avg Purchase (৳)', value: `৳${avg.toFixed(2)}`, icon: Hash, grad: 'from-yellow-500 to-yellow-600' },
    ]
  }, [items, meta])
    ...
    const handleExpandClick = async (e: Event) => {
      ...
      details.innerHTML = `
        <div class="bg-gradient-to-r from-blue-600 to-blue-500 text-white px-6 py-4">
          ...
          <p class="text-sm opacity-90">Purchase #${id}</p>
        </div>
        <div class="p-6">
          <div id="purchase-details-${id}" class="text-gray-500 text-sm">
      ...
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/api/inventory/purchases/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
          ...
          <div class="space-y-6">
            <div class="grid grid-cols-2 gap-x-8 gap-y-4 text-sm border-b pb-6">
              ...
              <div>
                ...
                <p class="font-semibold capitalize">${purchase.status || '-'}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p class="font-semibold text-gray-800">${purchase.product_name || '-'}</p>
              </div>
              <div>
                <p class="text-gray-500">Quantity</p>
                <p class="font-semibold text-gray-800">${purchase.quantity || '-'}</p>
              </div>
              <div>
                <p class="text-gray-500">Unit Price</p>
                <p class="font-semibold text-gray-800">৳${Number(purchase.unit_price || 0).toFixed(2)}</p>
              </div>
              <div>
                <p class="text-gray-500">Total Amount</p>
                <p class="font-semibold text-gray-800">৳${Number(purchase.total_amount || 0).toFixed(2)}</p>
              </div>
              <div>
                <p class="text-gray-500">Purchase Date</p>
                <p class="font-semibold text-gray-800">${formatDateTime(purchase.purchase_date)}</p>
              </div>
            </div>

            <div class="flex justify-end gap-3 pt-4 border-t">
              <a href="/dashboard/inventory/purchases/${id}/print"
                 class="inline-flex items-center justify-center rounded-lg text-sm font-medium border border-gray-300 bg-white hover:bg-gray-100 h-10 px-5 transition">
                Print Invoice
              </a>
              <a href="/dashboard/inventory/purchases/${id}"
        ...
        const container = document.getElementById(`purchase-details-${id}`);
        if (container) {
          container.innerHTML = purchaseDetailsHTML;
        }
      } catch (error) {
  ...
  const columns = [
    {
      ...
      title: "Purchase ID",
      orderable: true,
      render: (data: any, _type: string, row: PurchaseItem) => {
        const value = data || `PUR-${String(row.id).padStart(4, '0')}`;
        return `
    ...
    },
    {
      ...
      title: "Quantity",
      render: (data: number) => {
        return `<span class="font-mono text-xs font-medium">${Number(data || 0).toLocaleString()}</span>`
      },
    },
    {
      data: "unit_price",
      title: "Unit Price (৳)",
      render: (data: number) => {
        return `<span class="font-mono text-xs">৳${Number(data || 0).toFixed(2)}</span>`
      },
    },
    {
      data: "total_amount",
      title: "Total Amount (৳)",
      render: (data: number) => {
        return `<span class="font-semibold text-emerald-700 font-mono text-sm">৳${Number(data || 0).toFixed(2)}</span>`
      },
    },
    {
      data: "purchase_date",
    ...
    },
    {
      ...
      title: "Actions",
      orderable: false,
      render: (_data: any, _type: string, row: PurchaseItem) => {
        const id = row.id;
        return `<div class="flex gap-2">
          <a href="/dashboard/inventory/purchases/${id}/print" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded text-xs font-semibold shadow transition-colors">
          ...
          <a href="/dashboard/inventory/purchases/${id}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-green-600 hover:bg-green-700 text-white rounded text-xs font-semibold shadow transition-colors">
  ...
  return (
    <>
      <AppHeader
        ...
        description="Complete record of supplier purchases with filtering, search, and expandable details"
        fixed
      />
        ...
        <DataTable
          ...
          filterSlot={
            <div className="flex items-center gap-1.5">
              ...
              <Link
                to="/dashboard/reports/inventory/supplier-purchases/print"
                search={{
