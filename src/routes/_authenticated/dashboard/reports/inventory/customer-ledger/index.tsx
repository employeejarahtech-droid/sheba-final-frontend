...
import { BookOpen, DollarSign, Users, TrendingDown, Printer, FileText, Receipt } from 'lucide-react'
...
export const Route = createFileRoute('/_authenticated/dashboard/reports/inventory/customer-ledger/')({
...
function CustomerLedgerReport() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['inventory-customer-ledger', page, limit, search, from, to],
    queryFn: async () => {
  ...
  const items: CustomerLedgerItem[] = data?.data?.items || []
  ...
  const stats = useMemo(() => {
    ...
    return [
      { label: 'Total Customers', value: items.length, icon: BookOpen, grad: 'from-green-500 to-green-600' },
      { label: 'Total Billed (৳)', value: `৳${totalBilled.toLocaleString()}`, icon: DollarSign, grad: 'from-orange-500 to-orange-600' },
      { label: 'Total Collected (৳)', value: `৳${totalCollected.toLocaleString()}`, icon: Users, grad: 'from-pink-500 to-pink-600' },
      { label: 'Outstanding (৳)', value: `৳${outstanding.toLocaleString()}`, icon: TrendingDown, grad: 'from-teal-500 to-teal-600' },
    ]
  }, [items, meta])
  ...
  const columns = [
    {
      ...
      orderable: true,
      render: (data: any) => {
        return `<span class="font-mono text-xs text-purple-600 bg-purple-50 px-2 py-1 rounded font-semibold">${data || '-'}</span>`;
      },
    },
    {
      data: "patient_name",
    ...
    },
    {
      ...
      render: (_data: any, _type: string, row: CustomerLedgerItem) => {
        ...
        return `<div class="flex gap-2">
          <a href="/dashboard/outdoor-invoice/${id}/print" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded text-xs font-semibold shadow transition-colors">
          ...
          <a href="/dashboard/outdoor-invoice/${id}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-green-600 hover:bg-green-700 text-white rounded text-xs font-semibold shadow transition-colors">
  ...
  return (
    <>
      <AppHeader
        ...
        description="Customer billing and payment ledger with filtering, search, and detailed records"
        fixed
      />
      ...
      <main className="">
          ...
          {stats.map((stat, index) => {
            ...
            return (
              <Card key={index} className="overflow-hidden transition-all duration-300 gap-0 shadow-none p-0 border">
                <CardHeader className="border-b py-2 px-4 gap-0" style={{ backgroundColor: COLORS[index] }}>
        ...
        <DataTable
          tableTitle="Customer Ledger List"
          columns={columns}
          data={items}
          meta={meta}
          ...
          filterSlot={
            <div className="flex items-center gap-1.5">
              ...
              <Link
                to="/dashboard/reports/inventory/customer-ledger/print"
                search={{
          ...
          emptyState={
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 font-medium">No customer ledger records found</p>
