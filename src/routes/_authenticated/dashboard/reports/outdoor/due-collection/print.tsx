...
export const Route = createFileRoute('/_authenticated/dashboard/reports/outdoor/due-collection/print')({
  validateSearch: searchSchema,
  component: DueCollectionPrint,
})
...
function DueCollectionPrint() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['print-outdoor-due-collection', search, start_date, end_date],
    queryFn: async () => {
      ...
      if (!res.ok) throw new Error('Failed to fetch due collection data')
      return res.json()
    },
    enabled: !!token,
  })
  ...
  const allItems = useMemo(() => data?.data?.items ?? [], [data?.data?.items])

  // Filter only items with due amount > 0
  const items = useMemo(() =>
    allItems.filter((r: any) => (Number(r.bill_amount || 0) - Number(r.collected_amount || 0)) > 0),
    [allItems]
  )
  ...
  const stats = useMemo(() => {
    const totalDue = flatItems.reduce((sum: number, r: any) => sum + (Number(r.bill_amount || 0) - Number(r.collected_amount || 0)), 0)
    const totalBilled = flatItems.reduce((sum: number, r: any) => sum + Number(r.bill_amount || 0), 0)
    const avgDue = flatItems.length > 0 ? totalDue / flatItems.length : 0

    return [
      { label: 'Due Invoices', value: flatItems.length },
      { label: 'Total Due Amount', value: `৳${totalDue.toLocaleString()}` },
      { label: 'Avg Due Amount', value: `৳${Math.round(avgDue).toLocaleString()}` },
      { label: 'Total Billed', value: `৳${totalBilled.toLocaleString()}` },
      { label: 'Records in Period', value: flatItems.length },
      { label: 'Collection Rate', value: `${totalBilled > 0 ? Math.round(((totalBilled - totalDue) / totalBilled) * 100) : 0}%` },
    ]
  }, [flatItems])
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-screen gap-3">
        ...
        <span className="text-gray-600">Loading due collection data...</span>
      </div>
    )
  }
    ...
    <div className="max-w-6xl mx-auto w-full p-8 bg-white">
      ...
      <div className="mb-2 flex items-start justify-between gap-6">
        ...
        <div className="w-1/2 text-right">
          <h2 className="text-lg font-bold tracking-widest uppercase">Due Collection Report</h2>
          <p className="text-xs text-gray-600 mt-1">Outdoor invoices with outstanding balances</p>
      ...
      <table className="w-full text-xs mt-2" style={{ tableLayout: 'fixed' }}>
        <thead>
          <tr className="border-t border-b" style={{ backgroundColor: '#f0f9ff' }}>
            ...
            <th className="px-1.5 py-1 text-left text-[10px]" style={{ width: '11%' }}>Invoice No</th>
            ...
            <th className="px-1.5 py-1 text-left text-[10px]" style={{ width: '12%' }}>Department</th>
            <th className="px-1.5 py-1 text-right text-[10px]" style={{ width: '11%' }}>Bill (৳)</th>
            <th className="px-1.5 py-1 text-right text-[10px]" style={{ width: '11%' }}>Paid (৳)</th>
            <th className="px-1.5 py-1 text-right text-[10px]" style={{ width: '12%' }}>Due (৳)</th>
            <th className="px-1.5 py-1 text-left text-[10px]" style={{ width: '12%' }}>Date</th>
          </tr>
        </thead>
        <tbody>
          {flatItems.map((item: DueCollectionItem, idx: number) => {
            const dueAmount = (Number(item.bill_amount || 0) - Number(item.collected_amount || 0))
            return (
              <tr key={item.id || idx} className="border-b border-dashed">
                <td className="px-1.5 py-1 text-[10px] text-gray-500">{idx + 1}</td>
                <td className="px-1.5 py-1 text-[10px] font-mono">{item.invoice_no || `INV-${String(item.id).padStart(4, '0')}`}</td>
                <td className="px-1.5 py-1 text-[10px] font-medium">{item.patient_name || '-'}</td>
                <td className="px-1.5 py-1 text-[10px] font-mono">{item.patient_phone || item.phone || '-'}</td>
                ...
                <td className="px-1.5 py-1 text-[10px] text-right">{Number(item.collected_amount || 0).toFixed(2)}</td>
                <td className="px-1.5 py-1 text-[10px] text-right font-semibold" style={{ color: '#dc2626' }}>{dueAmount.toFixed(2)}</td>
          ...
          {flatItems.length === 0 && (
            <tr>
              <td colSpan={9} className="px-2 py-4 text-center text-gray-500" style={{ color: '#6b7280 !important' }}>
                <div className="font-semibold text-xs">No due invoices found for the selected criteria</div>
