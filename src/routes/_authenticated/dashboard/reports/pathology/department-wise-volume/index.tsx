...
import { Building2, Layers, TestTube2, FileText, Printer, Hash } from 'lucide-react'
...
export const Route = createFileRoute('/_authenticated/dashboard/reports/pathology/department-wise-volume/')({
  component: DepartmentWiseVolumeReport,
})

function DepartmentWiseVolumeReport() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['pathology-dept-volume', page, limit, search, from, to],
    queryFn: async () => {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/department?${params}`, {
  ...
  const { data: statsData } = useQuery({
    queryKey: ['dept-statistics-overall'],
    queryFn: async () => {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/department/statistics/overall`, {
        headers: { Authorization: `Bearer ${token}` },
      })
  ...
  const statistics = useMemo(() => [
    { label: 'Total Departments', value: stats.totalDepartments || 0, icon: Building2, grad: 'from-green-500 to-green-600' },
    { label: 'Total Categories', value: stats.totalCategories || 0, icon: Layers, grad: 'from-orange-500 to-orange-600' },
    { label: 'Total Tests', value: stats.totalTests || 0, icon: TestTube2, grad: 'from-pink-500 to-pink-600' },
    { label: 'Total Reports', value: stats.totalReports || 0, icon: FileText, grad: 'from-teal-500 to-teal-600' },
  ...
  const columns = [
    {
      ...
      title: "#",
      orderable: false,
      render: (_data: any, _type: string, row: DepartmentItem, m: any) => String((m.row ?? 0) + 1 + (page - 1) * limit),
    },
    {
      data: "name",
    ...
    },
    {
      ...
      render: (_data: any, _type: string, row: DepartmentItem) => {
        ...
        return `<div class="flex gap-2">
          <a href="/dashboard/reports/pathology/department-wise-volume/print"
          ...
          <a href="/dashboard/pathology/departments/${id}"
  ...
  return (
    <>
      <AppHeader
        title="Department-wise Volume Report"
        description="Overview of all departments and their test volumes"
        fixed
      />
      ...
      <main className="">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 mb-6">
          {statistics.map((stat, index) => {
            ...
            const colors = ['#10B981', '#F97316', '#EC4899', '#14B8A6', '#F59E0B']
            return (
              <Card key={index} className="overflow-hidden transition-all duration-300 gap-0 shadow-none p-0 border">
                <CardHeader className="border-b py-2 px-4 gap-0" style={{ backgroundColor: colors[index % 5] }}>
                  <div className="flex items-center gap-2.5">
                    <div className="p-2 bg-white rounded-lg shadow-lg">
                      <Icon className="w-4 h-4" style={{ color: colors[index % 5] }} />
                    </div>
        ...
        <DataTable
          ...
          filterSlot={
            <div className="flex items-center gap-1.5">
              ...
              <Link
                to="/dashboard/reports/pathology/department-wise-volume/print"
                search={{
