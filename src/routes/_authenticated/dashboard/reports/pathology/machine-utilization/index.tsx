...
import { Monitor, CheckCircle2, Clock, Activity, Printer, FileText, Hash } from 'lucide-react'
...
interface MachineUtilizationItem {
  id: number
  ReceiptID: string
  PatientName: string
  Tests: string
...
export const Route = createFileRoute('/_authenticated/dashboard/reports/pathology/machine-utilization/')({
  component: MachineUtilizationPage,
})

function MachineUtilizationPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['machine-utilization', page, limit, search, from, to],
    queryFn: async () => {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/ecg-all?${params}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      if (!res.ok) throw new Error('Failed to fetch machine utilization data')
      return res.json()
    },
    enabled: !!token,
  ...
  const items: MachineUtilizationItem[] = data?.data?.items || []
  ...
  const stats = useMemo(() => {
    ...
    const utilizationRate = items.length > 0 ? Math.round((completed / items.length) * 100) : 0

    return [
      { label: 'Total Records', value: meta.total, icon: Monitor, grad: 'from-green-500 to-green-600' },
      { label: 'Completed Tests', value: completed, icon: CheckCircle2, grad: 'from-blue-500 to-blue-600' },
      { label: 'Pending Tests', value: pending, icon: Clock, grad: 'from-orange-500 to-orange-600' },
      { label: 'This Page', value: items.length, icon: Hash, grad: 'from-teal-500 to-teal-600' },
      { label: 'Utilization Rate', value: `${utilizationRate}%`, icon: Activity, grad: 'from-pink-500 to-pink-600' },
      { label: 'Today Records', value: items.filter(i => i.Date && new Date(i.Date).toDateString() === new Date().toDateString()).length, icon: Monitor, grad: 'from-yellow-500 to-yellow-600' },
    ]
  }, [items, meta])
    ...
    const handleExpandClick = async (e: Event) => {
      ...
      details.innerHTML = `
        <div class="bg-gradient-to-r from-purple-600 to-purple-500 text-white px-6 py-4">
          <h2 class="text-xl font-semibold">Machine Utilization Details</h2>
          <p class="text-sm opacity-90">Receipt #${receiptId}</p>
        </div>
        <div class="p-6">
          <div id="machine-details-${id}" class="text-gray-500 text-sm">
            Loading details...
          </div>
        </div>
      `;
      ...
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/api/ecg-all/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
          ...
          <div class="space-y-6">
            <div class="grid grid-cols-2 gap-x-8 gap-y-4 text-sm border-b pb-6">
              <div>
                ...
                <p class="font-semibold text-gray-800">${item?.ReceiptID || receiptId || '-'}</p>
              </div>
              <div>
                <p class="text-gray-500">Status</p>
                <p class="font-semibold capitalize">${item?.Status || item?.status || '-'}</p>
              </div>
              <div>
                <p class="text-gray-500">Patient Name</p>
                <p class="font-semibold text-gray-800">${item?.PatientName || '-'}</p>
              </div>
              <div>
                <p class="text-gray-500">Test Date</p>
                <p class="font-semibold text-gray-800">${formatDateTime(item?.Date)}</p>
              </div>
              <div class="col-span-2">
                <p class="text-gray-500">Tests</p>
                <p class="font-semibold text-gray-800">${item?.Tests || '-'}</p>
              </div>
            </div>

            <div class="flex justify-end gap-3 pt-4 border-t">
              <a href="/dashboard/reports/pathology/machine-utilization/print?search=${encodeURIComponent(receiptId)}"
        ...
        const container = document.getElementById(`machine-details-${id}`);
        if (container) {
  ...
  const columns = [
    {
      ...
      title: "Receipt ID",
      orderable: true,
      render: (data: any, _type: string, row: MachineUtilizationItem) => {
        const value = data || `REC-${String(row.id).padStart(4, '0')}`;
        return `
    ...
    },
    {
      ...
      title: "Tests",
      render: (data: string) => {
        return `<span class="text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded max-w-xs block truncate">${data || '-'}</span>`
      },
    },
    {
      data: "Date",
    ...
    },
    {
      ...
      title: "Actions",
      orderable: false,
      render: (_data: any, _type: string, row: MachineUtilizationItem) => {
        const id = row.id;
        const receiptId = row.ReceiptID || `REC-${String(row.id).padStart(4, '0')}`;
        return `<div class="flex gap-2">
          <a href="/dashboard/reports/pathology/machine-utilization/print?search=${encodeURIComponent(receiptId)}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded text-xs font-semibold shadow transition-colors">
          ...
          <button onclick="document.querySelector('.expand-btn[data-id=\\'${id}\\']')?.click()" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-green-600 hover:bg-green-700 text-white rounded text-xs font-semibold shadow transition-colors">
  ...
  return (
    <>
      <AppHeader
        ...
        description="Equipment usage based on ECG examination records with filtering, search, and expandable details"
        fixed
      />
      ...
      <main className="">
        ...
        <DataTable
          tableTitle="Machine Utilization Records"
          columns={columns}
          data={items}
          meta={meta}
          ...
          filterSlot={
            <div className="flex items-center gap-1.5">
              ...
              <Link
                to="/dashboard/reports/pathology/machine-utilization/print"
                search={{
          ...
          emptyState={
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 font-medium">No machine utilization records found</p>
