...
export const Route = createFileRoute('/_authenticated/dashboard/reports/pathology/machine-utilization/print')({
  validateSearch: searchSchema,
  component: MachineUtilizationPrint,
})
...
function MachineUtilizationPrint() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['print-machine-utilization', search, start_date, end_date],
    queryFn: async () => {
      const params = new URLSearchParams({ page: '1', limit: '9999', search: search ?? '' })
      if (start_date) params.set('start_date', start_date)
      if (end_date) params.set('end_date', end_date)
      const res = await fetch(`${API_URL}/api/ecg-all?${params}`, {
  ...
  const stats = useMemo(() => {
    ...
    const completed = flatItems.filter((r: any) => r.Status === 'Completed' || r.status === 'Completed').length
    const pending = total - completed
    const rate = total > 0 ? Math.round((completed / total) * 100) : 0
    const totalFromMeta = data?.data?.meta?.total ?? flatItems.length
    const todayRecords = flatItems.filter((r: any) => r.Date && new Date(r.Date).toDateString() === new Date().toDateString()).length

    return [
      { label: 'Total Records', value: totalFromMeta, color: COLORS[0] },
      { label: 'Completed Tests', value: completed, color: COLORS[1] },
      { label: 'Pending Tests', value: pending, color: COLORS[2] },
      { label: 'Utilization Rate', value: `${rate}%`, color: COLORS[5] },
      { label: 'Today Records', value: todayRecords, color: COLORS[4] },
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-screen gap-3">
        ...
        <span className="text-gray-600">Loading machine utilization data...</span>
      </div>
    )
  }
    ...
    <div className="max-w-6xl mx-auto w-full p-8 bg-white">
      ...
      <div className="mb-2 flex items-start justify-between gap-6">
        ...
        <div className="w-1/2 text-right">
          <h2 className="text-lg font-bold tracking-widest uppercase">Machine Utilization Report</h2>
          <p className="text-xs text-gray-600 mt-1">Equipment usage based on ECG examination records</p>
      ...
      <table className="w-full text-xs mt-2">
        <thead>
          <tr className="border-t border-b" style={{ backgroundColor: '#f0f9ff' }}>
            ...
            <th className="px-1.5 py-1 text-left text-[10px] w-[10%]">Receipt ID</th>
            <th className="px-1.5 py-1 text-left text-[10px] w-[18%]">Patient Name</th>
            <th className="px-1.5 py-1 text-left text-[10px] w-[22%]">Tests</th>
            <th className="px-1.5 py-1 text-left text-[10px] w-[10%]">Test Date</th>
            <th className="px-1.5 py-1 text-center text-[10px] w-[10%]">Status</th>
            <th className="px-1.5 py-1 text-left text-[10px] w-[26%]">Remarks</th>
          </tr>
        </thead>
        <tbody>
          {flatItems.map((item: MachineUtilizationItem, idx: number) => (
            <tr key={item.id || idx} className="border-b border-dashed">
              <td className="px-1.5 py-1 text-[10px] text-gray-500">{idx + 1}</td>
              <td className="px-1.5 py-1 text-[10px] font-mono font-semibold text-purple-600">{item.ReceiptID || '-'}</td>
              <td className="px-1.5 py-1 text-[10px] font-medium">{item.PatientName || '-'}</td>
              ...
              <td className="px-1.5 py-1 text-[10px] text-gray-500 italic">ECG Machine</td>
            </tr>
          ))}
          {flatItems.length === 0 && (
            <tr>
              <td colSpan={7} className="px-2 py-4 text-center text-gray-500" style={{ color: '#6b7280 !important' }}>
                <div className="font-semibold text-xs">No machine utilization records found for the selected criteria</div>
