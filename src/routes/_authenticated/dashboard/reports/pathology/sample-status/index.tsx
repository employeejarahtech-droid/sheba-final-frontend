...
import { FlaskConical, CheckCircle2, Clock, TestTube2, Printer, FileText, Calendar } from 'lucide-react'
...
export const Route = createFileRoute('/_authenticated/dashboard/reports/pathology/sample-status/')({
  component: SampleStatusReport,
})
...
function SampleStatusReport() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['pathology-sample-status', page, limit, search, from, to],
    queryFn: async () => {
  ...
  const items: SampleStatusItem[] = data?.data?.items || []
  const meta: Meta = data?.data?.meta || { total: 0, page: 1, limit: 10, totalPages: 1 }

  // Calculate statistics
  const stats = useMemo(() => {
    const processed = items.filter((i) => Number(i.collected_amount || 0) > 0).length
    const pending = items.length - processed
    const uniqueDepts = new Set(items.map((i) => i.department_name).filter(Boolean)).size

    return [
      { label: 'Total Samples', value: meta.total, icon: FlaskConical, grad: 'from-green-500 to-green-600' },
      { label: 'Processed', value: processed, icon: CheckCircle2, grad: 'from-blue-500 to-blue-600' },
      { label: 'Pending', value: pending, icon: Clock, grad: 'from-orange-500 to-orange-600' },
      { label: 'Departments', value: uniqueDepts, icon: TestTube2, grad: 'from-pink-500 to-pink-600' },
    ]
  }, [items, meta])
  ...
  const columns = [
    ...
    },
    {
      ...
      orderable: false,
      render: (data: string) => {
        const isCompleted = data === 'Completed' || data === 'completed'
        return `<span class="px-2 py-0.5 rounded text-xs font-medium ${isCompleted ? 'bg-emerald-100 text-emerald-700' : 'bg-yellow-100 text-yellow-700'}">${data || '-'}</span>`
      },
    },
    {
      data: "created_at",
    ...
    },
    {
      ...
      title: "Actions",
      orderable: false,
      render: (_data: any, _type: string, row: SampleStatusItem) => {
        const id = row.id;
        return `<div class="flex gap-2">
          <a href="/dashboard/billing/outdoor-invoice/${id}/print" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded text-xs font-semibold shadow transition-colors">
          ...
          <a href="/dashboard/billing/outdoor-invoice/${id}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-green-600 hover:bg-green-700 text-white rounded text-xs font-semibold shadow transition-colors">
  ...
  return (
    <>
      <AppHeader
        ...
        description="Sample processing status by invoice with filtering and search"
        fixed
      />
        ...
        <DataTable
          ...
          filterSlot={
            <div className="flex items-center gap-1.5">
              ...
              <Link
                to="/dashboard/reports/pathology/sample-status/print"
                search={{
          ...
          emptyState={
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 font-medium">No sample records found</p>
