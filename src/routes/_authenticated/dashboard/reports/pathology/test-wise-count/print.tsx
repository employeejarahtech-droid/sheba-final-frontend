...
function TestWiseCountPrint() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['print-test-wise-count', search, start_date, end_date],
    queryFn: async () => {
  ...
  const stats = useMemo(() => {
    const completedCount = flatItems.filter((r: any) => r.status === 'Completed' || r.status === 'completed').length
    const pendingCount = flatItems.filter((r: any) => r.status !== 'Completed' && r.status !== 'completed').length
    ...
    return [
      { label: 'Total Tests', value: totalFromMeta },
      { label: 'Completed', value: completedCount },
      { label: 'Pending', value: pendingCount },
      { label: 'Departments', value: uniqueDepts },
  ...
  return (
    <div className="max-w-6xl mx-auto w-full p-8 bg-white">
      ...
      <p className="text-center text-xs text-gray-600 mb-2">Count of tests grouped by department</p>
      ...
      <table className="w-full text-xs mt-2" style={{ tableLayout: 'fixed' }}>
        <thead>
          <tr className="border-t border-b" style={{ backgroundColor: '#f0f9ff' }}>
            ...
            <th className="px-1.5 py-1 text-left text-[10px]" style={{ width: '28%' }}>Tests</th>
            <th className="px-1.5 py-1 text-center text-[10px]" style={{ width: '12%' }}>Status</th>
            <th className="px-1.5 py-1 text-left text-[10px]" style={{ width: '15%' }}>Date</th>
          </tr>
        </thead>
        <tbody>
          {flatItems.map((item: TestWiseItem, idx: number) => (
            <tr key={item.id || idx} className="border-b border-dashed">
              ...
              <td className="px-1.5 py-1 text-[10px] text-center">
                <span className={`px-2 py-0.5 rounded text-[9px] font-medium ${(item.status === 'Completed' || item.status === 'completed') ? 'bg-emerald-100 text-emerald-700' : 'bg-yellow-100 text-yellow-700'}`}>
                  {item.status || '-'}
                </span>
              </td>
