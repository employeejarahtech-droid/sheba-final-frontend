...
import { Wallet, DollarSign, TrendingUp, CheckCircle2, Printer, FileText, Calendar } from 'lucide-react'
...
function AdvancePaymentsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['advance-payments', page, limit, search, from, to],
    queryFn: async () => {
  ...
  const columns = [
    {
      ...
      render: (data: any, _type: string, row: AdvancePaymentItem) => {
        const value = data || `ADV-${String(row.id).padStart(4, '0')}`;
        return `
    ...
    },
    {
      ...
      render: (_data: any, _type: string, row: AdvancePaymentItem) => {
        ...
        else if (pctNum >= 50) colorClass = 'bg-blue-100 text-blue-700';
        else if (pctNum > 0) colorClass = 'bg-yellow-100 text-yellow-700';
        else colorClass = 'bg-red-100 text-red-700';
        return `<span class="px-2 py-0.5 rounded-full text-xs font-semibold ${colorClass}">${pct}%</span>`;
      },
    },
    {
      data: null,
      ...
      render: (_data: any, _type: string, row: AdvancePaymentItem) => {
        ...
        return `<div class="flex gap-2">
          <a href="/dashboard/reports/indoor/advance-payments/print"
  ...
  return (
    <>
      <AppHeader
        title="Advance Payments Report"
        description="Track advance payments made by patients for indoor admissions"
        fixed
      />
      ...
      <main className="">
        ...
        <DataTable
          tableTitle="Advance Payments List"
          columns={columns}
          data={items}
          meta={meta}
          ...
          filterSlot={
            <div className="flex items-center gap-1.5">
              ...
              <Link
                to="/dashboard/reports/indoor/advance-payments/print"
                search={{
          ...
          emptyState={
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 font-medium">No advance payment records found</p>
