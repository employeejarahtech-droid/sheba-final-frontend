...
export const Route = createFileRoute('/_authenticated/dashboard/reports/indoor/advance-payments/print')({
  validateSearch: searchSchema,
  component: AdvancePaymentsPrint,
})
...
function AdvancePaymentsPrint() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['print-indoor-advance-payments', search, start_date, end_date],
    queryFn: async () => {
      ...
      if (!res.ok) throw new Error('Failed to fetch advance payments data')
      return res.json()
    },
    enabled: !!token,
  })
  ...
  const stats = useMemo(() => {
    const totalAdvance = flatItems.reduce((sum: number, r: any) => sum + Number(r.advance_payment || 0), 0)
    const avgAdvance = flatItems.length > 0 ? Math.round(totalAdvance / flatItems.length) : 0
    const fullyCovered = flatItems.filter((r: any) => Number(r.advance_payment || 0) >= Number(r.total_bill || 0)).length
    const totalFromMeta = data?.data?.meta?.total ?? flatItems.length

    return [
      { label: 'Total Admissions', value: totalFromMeta },
      { label: 'Total Advance Paid', value: `৳${totalAdvance.toLocaleString()}` },
      { label: 'Avg Advance', value: `৳${avgAdvance.toLocaleString()}` },
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-screen gap-3">
        ...
        <span className="text-gray-600">Loading advance payment data...</span>
      </div>
    )
  }
    ...
    <div className="max-w-6xl mx-auto w-full p-8 bg-white">
      ...
      <div className="mb-2 flex items-start justify-between gap-6">
        ...
        <div className="w-1/2 text-right">
          <h2 className="text-lg font-bold tracking-widest uppercase">Advance Payments Report</h2>
          <p className="text-xs text-gray-600 mt-1">Indoor advance payment details</p>
      ...
      <table className="w-full text-xs mt-2" style={{ tableLayout: 'fixed' }}>
        <thead>
          <tr className="border-t border-b" style={{ backgroundColor: '#f0f9ff' }}>
            ...
            <th className="px-1.5 py-1 text-left text-[10px]" style={{ width: '12%' }}>Admission No</th>
            <th className="px-1.5 py-1 text-left text-[10px]" style={{ width: '18%' }}>Patient Name</th>
            <th className="px-1.5 py-1 text-left text-[10px]" style={{ width: '13%' }}>Admission Date</th>
            <th className="px-1.5 py-1 text-right text-[10px]" style={{ width: '13%' }}>Total Bill (৳)</th>
            <th className="px-1.5 py-1 text-right text-[10px]" style={{ width: '13%' }}>Advance Paid (৳)</th>
            <th className="px-1.5 py-1 text-right text-[10px]" style={{ width: '13%' }}>Remaining (৳)</th>
            <th className="px-1.5 py-1 text-center text-[10px]" style={{ width: '13%' }}>Coverage %</th>
          </tr>
        </thead>
        <tbody>
          {flatItems.map((item: AdvancePaymentItem, idx: number) => {
            const remaining = (item.total_bill || 0) - (item.advance_payment || 0)
            const coverage = item.total_bill > 0 ? ((item.advance_payment || 0) / item.total_bill * 100).toFixed(1) : '0.0'
            return (
              <tr key={item.id || idx} className="border-b border-dashed">
                <td className="px-1.5 py-1 text-[10px] text-gray-500">{idx + 1}</td>
                <td className="px-1.5 py-1 text-[10px] font-mono">{item.admission_no || `ADV-${String(item.id).padStart(4, '0')}`}</td>
                ...
                <td className="px-1.5 py-1 text-[10px] text-right font-semibold" style={{ color: '#16a34a' }}>{(item.advance_payment || 0).toFixed(2)}</td>
                <td className="px-1.5 py-1 text-[10px] text-right" style={{ color: '#dc2626' }}>{remaining.toFixed(2)}</td>
                <td className="px-1.5 py-1 text-[10px] text-center font-mono">{coverage}%</td>
              </tr>
            )
          })}
          {flatItems.length === 0 && (
            <tr>
              <td colSpan={8} className="px-2 py-4 text-center text-gray-500" style={{ color: '#6b7280 !important' }}>
                <div className="font-semibold text-xs">No advance payment records found for the selected criteria</div>
