...
export const Route = createFileRoute('/_authenticated/dashboard/reports/indoor/assistant-bill/print')({
  ...
  component: AssistantBillPrint,
})

interface AssistantBillItem {
  id: number
  admission_id: number
  service_name: string
...
function AssistantBillPrint() {
  ...
  const { data, isLoading } = useQuery<{
    status: boolean
    data: { items: AssistantBillItem[]; meta: Meta }
    summary: {
      totalPayable: number
      totalDue: number
    ...
    queryKey: ['print-assistant-bill', search, from, to],
    queryFn: async () => {
      const params = new URLSearchParams({ page: '1', limit: '9999', search: search ?? '' })
      if (from) params.set('from', from)
      if (to) params.set('to', to)
      const res = await fetch(`${API_URL}/api/bill-distribution/final/assistant?${params}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      if (!res.ok) throw new Error('Failed to fetch assistant bill data')
      return res.json()
    },
    enabled: !!token,
  })
  ...
  const paidAmount = (r: AssistantBillItem) => Math.max(num(r.payable_now) - num(r.due_amount), 0)
  const providerName = (r: AssistantBillItem) => r.doctor?.doctor_name || r.service_name || 'Unspecified'
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-screen gap-3">
        ...
        <span className="text-gray-600">Loading assistant bill data...</span>
      </div>
    )
  }
    ...
    <div className="max-w-6xl mx-auto w-full p-8 bg-white">
      ...
      <div className="mb-2 flex items-start justify-between gap-6">
        ...
        <div className="w-1/2 text-right">
          <h2 className="text-lg font-bold tracking-widest uppercase">Assistant Bill Report</h2>
          <p className="text-xs text-gray-600 mt-1">Assistant-wise analysis of indoor assistant bill distributions</p>
      ...
      <table className="w-full text-xs mt-2" style={{ tableLayout: 'fixed' }}>
        <thead>
          <tr className="border-t border-b" style={{ backgroundColor: '#f0f9ff' }}>
            <th className="px-1.5 py-1 text-left text-[10px]" style={{ width: '25%' }}>Assistant</th>
          ...
          {providerRows.length === 0 && (
            <tr>
              <td colSpan={7} className="px-2 py-4 text-center text-gray-500" style={{ color: '#6b7280 !important' }}>
                <div className="font-semibold text-xs">No assistant bill records found for the selected criteria</div>
      ...
      <div className="mt-3 pt-2 border-t text-xs">
        <div className="grid grid-cols-2 gap-2">
          <div>
            <strong>Total Assistants:</strong> {providerRows.length}
          </div>
