...
export const Route = createFileRoute('/_authenticated/dashboard/reports/indoor/service-wise-revenue/print')({
  validateSearch: searchSchema,
  component: ServiceWiseRevenuePrint,
})
...
function ServiceWiseRevenuePrint() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['print-indoor-service-wise-revenue', search, start_date, end_date],
    queryFn: async () => {
      ...
      if (!res.ok) throw new Error('Failed to fetch service wise revenue data')
      return res.json()
    },
    enabled: !!token,
  })
  ...
  const stats = useMemo(() => {
    ...
    const activePatients = flatItems.filter((r: any) => r.status === 'active').length
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-screen gap-3">
        ...
        <span className="text-gray-600">Loading service revenue data...</span>
      </div>
    )
  }
    ...
    <div className="max-w-6xl mx-auto w-full p-8 bg-white">
      ...
      <div className="mb-2 flex items-start justify-between gap-6">
        ...
        <div className="w-1/2 text-right">
          <h2 className="text-lg font-bold tracking-widest uppercase">Service-Wise Revenue Report</h2>
          <p className="text-xs text-gray-600 mt-1">Indoor revenue analysis by ward/service</p>
      ...
      <table className="w-full text-xs mt-2" style={{ tableLayout: 'fixed' }}>
        <thead>
          <tr className="border-t border-b" style={{ backgroundColor: '#f0f9ff' }}>
            ...
            <th className="px-1.5 py-1 text-left text-[10px]" style={{ width: '14%' }}>Ward/Service</th>
            <th className="px-1.5 py-1 text-left text-[10px]" style={{ width: '11%' }}>Admission Date</th>
            <th className="px-1.5 py-1 text-right text-[10px]" style={{ width: '11%' }}>Total Bill (৳)</th>
            <th className="px-1.5 py-1 text-right text-[10px]" style={{ width: '11%' }}>Collected (৳)</th>
            <th className="px-1.5 py-1 text-right text-[10px]" style={{ width: '11%' }}>Due (৳)</th>
            <th className="px-1.5 py-1 text-left text-[10px]" style={{ width: '10%' }}>Status</th>
          </tr>
        </thead>
        <tbody>
          {flatItems.map((item: ServiceWiseRevenueItem, idx: number) => {
            const due = (item.total_bill || 0) - (item.advance_payment || 0)
            return (
              <tr key={item.id || idx} className="border-b border-dashed">
                ...
                <td className="px-1.5 py-1 text-[10px]">{item.ward_name || item.service_type || '-'}</td>
                ...
                <td className="px-1.5 py-1 text-[10px] text-right">{Number(item.advance_payment || 0).toFixed(2)}</td>
                <td className="px-1.5 py-1 text-[10px] text-right">{due.toFixed(2)}</td>
          ...
          {flatItems.length === 0 && (
            <tr>
              <td colSpan={9} className="px-2 py-4 text-center text-gray-500" style={{ color: '#6b7280 !important' }}>
                <div className="font-semibold text-xs">No service-wise revenue records found for the selected criteria</div>
