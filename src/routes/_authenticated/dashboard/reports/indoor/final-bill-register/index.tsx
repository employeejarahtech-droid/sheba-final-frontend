...
import { FileText, Receipt, DollarSign, TrendingDown, Printer, User, Calendar, Wallet, CheckCircle, XCircle } from 'lucide-react'
...
function FinalBillRegisterPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['final-bill-register', page, limit, search, from, to],
    queryFn: async () => {
    ...
    const handleExpandClick = async (e: Event) => {
      ...
      details.innerHTML = `
        <div class="bg-gradient-to-r from-purple-600 to-purple-500 text-white px-6 py-4">
          <h2 class="text-xl font-semibold">Final Bill Details</h2>
          <p class="text-sm opacity-90">Bill #${id}</p>
        </div>
        <div class="p-6">
          <div id="bill-details-${id}" class="text-gray-500 text-sm">
      ...
      try {
          ...
          <div class="space-y-6">
            <div class="grid grid-cols-2 gap-x-8 gap-y-4 text-sm border-b pb-6">
              <div>
                ...
                <p class="font-semibold text-gray-800">${bill.admission_prefix || bill.id || '-'}</p>
              </div>
              <div>
                <p class="text-gray-500">Payment Status</p>
                <p class="font-semibold capitalize">
                  <span class="px-2 py-0.5 rounded-full text-xs font-medium ${isSettled ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}">
              ...
              <div>
                ...
                <p class="font-semibold text-gray-800">${bill.patient_name || '-'}</p>
              </div>
              <div>
                <p class="text-gray-500">Phone</p>
                <p class="font-semibold text-gray-800">${bill.phone || '-'}</p>
              </div>
              <div>
                <p class="text-gray-500">Age / Gender</p>
                <p class="font-semibold text-gray-800">${bill.age ? `${bill.age} yrs` : '-'} / ${bill.sex?.toUpperCase() || '-'}</p>
              </div>
              <div>
                <p class="text-gray-500">Doctor</p>
                <p class="font-semibold text-gray-800">${bill.doctor?.doctor_name || '-'}</p>
              </div>
              <div>
                <p class="text-gray-500">Admission Date</p>
                <p class="font-semibold text-gray-800">${formatDateTime(bill.admission_date, bill.admission_time)}</p>
              </div>
              <div>
                <p class="text-gray-500">Discharge Date</p>
                <p class="font-semibold text-gray-800">${formatDateTime(bill.discharge_date, bill.discharge_time)}</p>
              </div>
              <div class="col-span-2">
                <p class="text-gray-500">Bed/Cabin</p>
                <p class="font-semibold text-gray-800">${bill.bedCabin ? `${bill.bedCabin.code} (${bill.bedCabin.type}) - ${bill.bedCabin.ward}` : '-'}</p>
              </div>
            </div>
        ...
        const container = document.getElementById(`bill-details-${id}`);
        if (container) {
          container.innerHTML = billDetailsHTML;
        }
      } catch (error) {
  ...
  return (
    <>
      <AppHeader
        ...
        description="Complete record of discharged patient final bills with filtering, search, and expandable details"
        fixed
      />
      ...
      <main className="">
        ...
        <DataTable
          tableTitle="Final Bill Register List"
          columns={columns}
          data={items}
          meta={meta}
          ...
          filterSlot={
            <div className="flex items-center gap-1.5">
              ...
              <Link
                to="/dashboard/reports/indoor/final-bill-register/print"
                search={{
          ...
          emptyState={
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 font-medium">No final bills found</p>
