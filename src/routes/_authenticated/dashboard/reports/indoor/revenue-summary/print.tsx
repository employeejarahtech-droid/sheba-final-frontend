...
export const Route = createFileRoute('/_authenticated/dashboard/reports/indoor/revenue-summary/print')({
  validateSearch: searchSchema,
  component: IndoorRevenueSummaryPrint,
})
...
function IndoorRevenueSummaryPrint() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['print-indoor-revenue-summary', search, start_date, end_date],
    queryFn: async () => {
      ...
      <table className="w-full text-xs mt-2" style={{ tableLayout: 'fixed' }}>
        <thead>
          <tr className="border-t border-b" style={{ backgroundColor: '#f0f9ff' }}>
            ...
            <th className="px-1.5 py-1 text-left text-[10px]" style={{ width: '11%' }}>Discharge Date</th>
            ...
            <th className="px-1.5 py-1 text-center text-[10px]" style={{ width: '13%' }}>Status</th>
          </tr>
        </thead>
        <tbody>
          ...
          {flatItems.length === 0 && (
            <tr>
              <td colSpan={9} className="px-2 py-4 text-center text-gray-500" style={{ color: '#6b7280 !important' }}>
                <div className="font-semibold text-xs">No revenue records found for the selected criteria</div>
