...
import { Users, Banknote, DollarSign, AlertCircle, Printer, FileText, BedDouble } from 'lucide-react'
...
function IndoorRevenueSummaryPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['indoor-revenue-summary', page, limit, search, from, to],
    queryFn: async () => {
      ...
      if (!res.ok) throw new Error('Failed to fetch indoor revenue summary')
      return res.json()
    },
    enabled: !!token,
  ...
  const getStatusBadge = (status: string) => {
    ...
    if (s === 'discharged' || s === 'paid') {
      return `<span class="px-3 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-700 border border-green-200 capitalize">${status}</span>`
    }
    if (s === 'active') {
      return `<span class="px-3 py-1 rounded-full text-xs font-semibold bg-yellow-100 text-yellow-700 border border-yellow-200 capitalize">${status}</span>`
    }
    return `<span class="px-3 py-1 rounded-full text-xs font-semibold bg-red-100 text-red-700 border border-red-200 capitalize">${status}</span>`
  };
  ...
  const columns = [
    {
      ...
      render: (data: any, _type: string, row: AdmissionItem) => {
        ...
        return `<span class="font-mono text-xs text-purple-600 bg-purple-50 px-2 py-1 rounded font-semibold">${value}</span>`;
      },
    },
    {
      data: "patient_name",
    ...
    },
    {
      ...
      orderable: true,
      render: (data: string | null) => {
        if (!data) return `<span class="text-gray-400 italic text-xs">Not discharged</span>`
        const date = new Date(data)
        return `<div class="text-sm">
          <div>${formatDate(date)}</div>
        </div>`
  ...
  return (
    <>
      <AppHeader
        ...
        description="Inpatient admission revenue overview with financial summaries"
        fixed
      />
      ...
      <main className="">
        ...
        <DataTable
          tableTitle="Indoor Revenue Summary List"
          columns={columns}
          data={items}
          meta={meta}
          ...
          filterSlot={
            <div className="flex items-center gap-1.5">
              ...
              <Link
                to="/dashboard/reports/indoor/revenue-summary/print"
                search={{
          ...
          emptyState={
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 font-medium">No revenue records found</p>
