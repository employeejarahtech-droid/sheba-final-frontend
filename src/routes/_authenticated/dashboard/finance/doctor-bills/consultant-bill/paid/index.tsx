...
import { z } from 'zod'
import { ConsultantBillPage } from '@/features/indoor/consultant-bill'

const consultantBillSearchSchema = z.object({
  page: z.coerce.number().catch(1),
  limit: z.coerce.number().catch(10),
  search: z.string().catch(''),
  from: z.string().catch(''),
  to: z.string().catch(''),
  status: z.string().catch('paid'),
...
export const Route = createFileRoute(
  ...
  validateSearch: (search) => consultantBillSearchSchema.parse(search),
  component: ConsultantBillPaidPageRoute,
})

function ConsultantBillPaidPageRoute() {
