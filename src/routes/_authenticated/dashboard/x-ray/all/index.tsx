...
export const Route = createFileRoute('/_authenticated/dashboard/x-ray/all/')({
...
function AllXRayReports() {
    const can = useCan();
    const canEdit = can('x-ray.all.edit');
  ...
  const { data: xrayAllReports, isFetching } = useQuery({
    queryKey: ["xray-all", page, limit, search, statusFilter, from, to, orderBy],
    queryFn: async () => {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/xray-all?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}${statusParam}${fromParam}${toParam}&orderBy=${encodeURIComponent(orderBy)}`,
        {
  ...
  useEffect(() => {
    const handleExpandClick = async (e: Event) => {
      ...
      // Fetch individual X-Ray test details from API
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_URL}/api/xray-all/${reciptId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
        if (!res.ok) throw new Error('Failed to fetch X-Ray details');

        const data = await res.json();
        const xrayTests = data.data?.xray_all_info || [];

        // Create test cards HTML
        const testCards = xrayTests.map((test: any) => `
          <div class="border rounded-xl p-4 hover:shadow-md transition bg-gray-50">
            ...
            <div class="flex gap-2 pt-3 border-t">
              ${canEdit ? `<a href="/dashboard/x-ray/all/edit/builder/${test.serial_id || test.id}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded text-xs font-semibold shadow transition-colors">
              ...
              <a href="/dashboard/x-ray/all/print/${test.serial_id || test.id}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-600 hover:bg-slate-700 text-white rounded text-xs font-semibold shadow transition-colors">
        ...
        let htmlContent = `
          <!-- Header -->
          <div class="bg-gradient-to-r from-slate-600 to-gray-700 text-white px-6 py-5">
            <div class="flex justify-between items-center">
              <div>
                <h2 class="text-xl font-semibold">X-Ray Examination Report</h2>
          ...
          <!-- Tests Section -->
          <div class="p-6">
            <h3 class="text-lg font-semibold mb-4 text-gray-800">X-Ray Examinations</h3>
            <div class="space-y-4">
    ...
    <>
      ...
      <main>
        <div className="p-4 space-y-3">
          ...
          <DataTable
            hideExport
            tableTitle="All Reports (X-Ray)"
            columns={columns}
            data={xrayAllReports?.data?.items || []}
            meta={{ page, limit, total: xrayAllReports?.data?.meta?.total || 0 }}
