...
function AllXRayReports() {
  ...
  const { data: xrayAllReports, isFetching } = useQuery({
    queryKey: ["xray-all", page, limit, search, statusFilter, from, to],
    queryFn: async () => {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/xray-all?page=${page}&limit=${limit}&search=${encodeURIComponent(search)}${statusParam}${fromParam}${toParam}`,
        {
