...
export const Route = createFileRoute(
    ...
    component: ProfitLossPrintPage,
});

function ProfitLossPrintPage() {
    const search = Route.useSearch();
    const [paddingTop, setPaddingTop] = useState(32);
    const paddingOptions = Array.from({ length: 39 }, (_, i) => (i + 2) * 5);

    const fromStr = search.from || format(startOfMonth(new Date()), 'yyyy-MM-dd');
    const toStr = search.to || format(new Date(), 'yyyy-MM-dd');
    const { currencySymbol } = useCurrency();

    const { data: pAndLData, isLoading } = useProfitAndLoss({ from: fromStr, to: toStr });

    const incomeData = pAndLData?.income || [];
    const expenseData = pAndLData?.expense || [];
    const totalIncome = pAndLData?.total_income || 0;
    const totalExpense = pAndLData?.total_expense || 0;
    const netProfit = pAndLData?.net_profit || 0;
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        ...
                        <div className="border-2 border-emerald-300">
                            <div className="bg-emerald-50 px-4 py-2 border-b-2 border-emerald-200">
                                <h2 className="font-bold text-emerald-700">INCOME</h2>
                            </div>
                        ...
                        <div className="border-2 border-red-300">
                            <div className="bg-red-50 px-4 py-2 border-b-2 border-red-200">
                                <h2 className="font-bold text-red-700">EXPENSES</h2>
                            </div>
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="bg-row-blue">
