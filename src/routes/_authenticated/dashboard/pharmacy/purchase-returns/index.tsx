...
import { usePurchaseReturnsQuery } from '@/features/pharmacy/pharmacyQueries'
...
export const Route = createFileRoute('/_authenticated/dashboard/pharmacy/purchase-returns/')({
...
function PurchaseReturnsPage() {
  ...
  const { data, isFetching } = usePurchaseReturnsQuery({ page, limit, search })
  ...
  const cards = useMemo(() => ([
    ...
    { label: 'Total Value (this page)', value: format(totalValue), icon: DollarSign, gradientClass: 'from-blue-500 to-indigo-500 shadow-blue-500/20' },
  ...
  useEffect(() => {
    const handleExpandClick = async (e: Event) => {
      ...
      try {
        const full = await pharmacyService.getPurchaseReturn(id)
    ...
    <>
      ...
      <main className="">
        <div className="flex flex-wrap items-end justify-between gap-2 mb-3">
          ...
          <Link to="/dashboard/pharmacy/purchase-returns/create"><Button>New Return</Button></Link>
        </div>
