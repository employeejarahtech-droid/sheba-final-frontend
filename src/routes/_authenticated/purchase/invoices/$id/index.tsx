...
} from "@/features/purchase/api/queries";
import {
    ArrowLeft,
    Printer,
    CreditCard,
...
export const Route = createFileRoute('/_authenticated/purchase/invoices/$id/')({
    component: PurchaseInvoicesDetails,
})

function PurchaseInvoicesDetails() {
    const { id } = Route.useParams();
    const currency = "BDT";

    const { data, isLoading } = useGetPurchaseInvoiceByIdQuery(id);
    const { mutate: markPaid, isPending: isMarkingPaid } = useUpdatePurchaseInvoiceMutation();
    ...
    return (
        <div className="max-w-7xl w-full mx-auto space-y-8 animate-in fade-in-50 duration-500 pb-10 p-6">

            {/* HEADER */}
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <div className="flex items-center gap-2 text-sm text-muted-foreground mb-1">
                        <Link to="/purchase/invoices" className="hover:text-primary transition-colors flex items-center gap-1">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    {/* <Link to={`/purchase-payments/create?pon=${invoice?.purchase_order.po_number}`}>
            <Button className="bg-indigo-600 hover:bg-indigo-700 text-white gap-2 shadow-sm">
              <CreditCard className="w-4 h-4" /> Record Payment
            </Button>
          </Link> */}

                    <Link to="/purchase/invoices/$id/preview" params={{ id: invoice?.id.toString() || '' }}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-4 space-y-6">
                    ...
                    {/* Quick Client Info */}
                    <Card className="shadow-sm border-border/60 overflow-hidden gap-2 pt-0">
                    ...
                    <Card className="shadow-md border-border/60 bg-slate-50 dark:bg-slate-900 overflow-hidden pt-0 gap-0">
