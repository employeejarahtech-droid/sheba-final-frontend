...
export function getAllPurchaseOrders({ page = 1, limit = 10, search = '' } = {}) {
    if (!db)
...
export function createPurchaseOrder(data) {
    ...
    return db.transaction(() => {
        ...
        if (!data.po_number) {
            let nextNum = 1;
            try {
                const result = db.prepare("SELECT seq FROM sqlite_sequence WHERE name = 'purchase_orders'").get();
                if (result) {
                ...
                const max = db.prepare('SELECT MAX(id) as m FROM purchase_orders').get();
                nextNum = (max.m || 0) + 1;
            }
        ...
        if ((!data.total_amount || data.total_amount === 0) && data.items && Array.isArray(data.items)) {
            ...
            data.items.forEach((item) => {
                ...
                if (item.purchase_tax && !item.tax_amount)
                    item.tax_amount = item.purchase_tax;
        ...
        `);
        const result = stmt.run(data.po_number, data.supplier_id, data.order_date, data.expected_delivery_date || null, data.status || 'pending', data.total_amount || 0, data.tax_amount || 0, data.discount_amount || 0, 0, // total_paid_amount
        ...
        if (data.items && Array.isArray(data.items)) {
            ...
            for (const item of data.items) {
                ...
                itemStmt.run(poId, item.product_id, qty, cost, disc, tax, lineTotal);
            }
        }
...
export function updatePurchaseOrder(id, data) {
    ...
    return db.transaction(() => {
        ...
        if (data.items && Array.isArray(data.items)) {
            ...
            `);
            for (const item of data.items) {
                itemStmt.run(id, item.product_id, item.quantity, item.unit_cost, item.discount || 0, item.tax_amount || 0, item.line_total);
            }
        }
...
export function getAllPurchasePayments({ page = 1, limit = 10, search = '' } = {}) {
    if (!db)
        throw new Error('Database not initialized');
    let query = `
        SELECT 
...
export function createPurchasePayment(data) {
    ...
    const createTransaction = db.transaction((pymtData) => {
        ...
        `);
        const result = insertPymt.run(pymtData.purchase_order_id, pymtData.amount, pymtData.payment_method, pymtData.reference_number, pymtData.notes, 'completed');
        const paymentId = result.lastInsertRowid;
        // 2. Update Purchase Order Paid Amount
        const po = db.prepare('SELECT total_paid_amount, total_amount, tax_amount, discount_amount FROM purchase_orders WHERE id = ?').get(pymtData.purchase_order_id);
        if (po) {
...
export function getPurchasePaymentById(id) {
    if (!db)
        throw new Error('Database not initialized');
    const stmt = db.prepare(`
        SELECT 
...
export function getAllCategories({ page = 1, limit = 1000, search = '' } = {}) {
    if (!db)
        throw new Error('Database not initialized');
    ...
    const countStmt = db.prepare(countQuery);
    const countResult = countStmt.get(...(search ? [`% ${search}% `, ` % ${search}% `] : []));
    const offset = (page - 1) * limit;
    query += ` LIMIT ? OFFSET ? `;
    params.push(limit, offset);
    const stmt = db.prepare(query);
...
export function createCategory(data) {
    if (!db)
        throw new Error('Database not initialized');
    const stmt = db.prepare('INSERT INTO categories (name, description, is_active) VALUES (?, ?, ?)');
    const result = stmt.run(data.name, data.description || null, data.is_active ? 1 : 0);
...
export function getAllUnits({ page = 1, limit = 1000, search = '' } = {}) {
    if (!db)
        throw new Error('Database not initialized');
    ...
    const countStmt = db.prepare(countQuery);
    const countResult = countStmt.get(...(search ? [`% ${search}% `] : []));
    const offset = (page - 1) * limit;
    query += ` LIMIT ? OFFSET ? `;
    params.push(limit, offset);
    const stmt = db.prepare(query);
    const units = stmt.all(...params);
    return {
...
export function createUnit(data) {
    if (!db)
        throw new Error('Database not initialized');
    const stmt = db.prepare('INSERT INTO units (name, is_active) VALUES (?, ?)');
    const result = stmt.run(data.name, data.is_active ? 1 : 0);
    return { id: result.lastInsertRowid, ...data };
}
...
export function getAllProducts({ page = 1, limit = 10, search = '' } = {}) {
    if (!db)
        throw new Error('Database not initialized');
    let query = `
        SELECT
...
export function createProduct(data) {
    ...
    `);
    const result = stmt.run(data.sku, data.name, data.description || null, data.category_id || null, data.unit_id || null, data.price, data.cost, data.initial_stock || 0, data.min_stock_level || 0, data.max_stock_level || 0, data.stock_quantity || 0, data.purchase_tax || 0, data.sales_tax || 0, data.weight || 0, data.width || 0, data.height || 0, data.length || 0, data.is_active ? 1 : 0, data.thumb_url || null, data.gallery_items ? JSON.stringify(data.gallery_items) : null, data.attributes ? JSON.stringify(data.attributes) : null);
    return { id: result.lastInsertRowid, ...data };
}
...
export function getProductStats() {
    ...
    const stockValueResult = stockValueStmt.get();
    return {
...
export function getAllAccounts({ page = 1, limit = 1000, search = '' } = {}) {
    if (!db)
        throw new Error('Database not initialized');
    ...
    const allAccounts = allAccountsStmt.all();
    ...
    if (search) {
        ...
        filteredList = flatList.filter(acc => acc.name.toLowerCase().includes(searchLower) ||
...
export function getAccountById(id) {
    ...
    while (parentId) {
        level++;
        const parent = db.prepare('SELECT parent_id FROM accounts WHERE id = ?').get(parentId);
        if (!parent)
            break;
...
export function createAccount(data) {
    ...
    if (!finalParentId) {
        if (data.type.toUpperCase() === 'INCOME' && data.code !== 'ROOT_INCOME') {
            const root = db.prepare('SELECT id FROM accounts WHERE name = ? OR code = ?').get('Income', 'ROOT_INCOME');
            if (root)
                finalParentId = root.id;
        }
        else if (data.type.toUpperCase() === 'EXPENSE' && data.code !== 'ROOT_EXPENSE') {
            const root = db.prepare('SELECT id FROM accounts WHERE name = ? OR code = ?').get('Expenses', 'ROOT_EXPENSE');
            if (root)
                finalParentId = root.id;
        }
    }
...
export function createJournalEntry(data) {
    if (!db)
        throw new Error('Database not initialized');
    return db.transaction(() => {
        const journalStmt = db.prepare('INSERT INTO journals (date, narration) VALUES (?, ?)');
...
export function getAccountingOverview() {
    ...
    const getTotalsForRange = (startDate, endDate) => {
        const query = `
            SELECT
...
// Reports
export function getJournalReport({ from, to } = {}) {
    if (!db)
        throw new Error('Database not initialized');
    let query = `
...
export function getTrialBalance(date) {
    if (!db)
        throw new Error('Database not initialized');
    const query = `
        ...
        GROUP BY a.id
        `;
    const rows = db.prepare(query).all(...(date ? [date] : []));
...
export function getProfitAndLoss({ from, to } = {}) {
    if (!db)
        throw new Error('Database not initialized');
    const query = `
        ...
        GROUP BY a.id
    `;
    const rows = db.prepare(query).all(...(from && to ? [from, to] : []));
    const report = {
        income: [],
        expense: [],
        total_income: 0,
...
export function createHeadWiseTransaction(data) {
    ...
    if (!headAccount)
        throw new Error(`${data.type} Head not found with ID: ${data.head_id} `);
    ...
    if (typeof data.payment_method === 'number') {
        ...
        paymentAccount = stmt.get(data.payment_method);
    }
    if (!paymentAccount) {
    ...
    return db.transaction(() => {
        ...
        const txStmt = db.prepare('INSERT INTO transactions (type, amount, payment_mode, date, description) VALUES (?, ?, ?, ?, ?)');
        ...
        const journalStmt = db.prepare('INSERT INTO journals (date, narration, reference_type, reference_id) VALUES (?, ?, ?, ?)');
...
export function getLedger({ account_id, from, to }) {
    if (!db)
        throw new Error('Database not initialized');
    // Get opening balance
    const openingQuery = `
    ...
    const openingBalance = db.prepare(openingQuery).get(...openingParams)?.balance || 0;
    // Get transactions
    let query = `
...
export function getAllTransactions({ from, to, type, search } = {}) {
    if (!db)
        throw new Error('Database not initialized');
    let query = `
...
export function getAllSuppliers({ page = 1, limit = 10, search = '' } = {}) {
    if (!db)
        throw new Error('Database not initialized');
    ...
    const countStmt = db.prepare(countQuery);
    const countResult = countStmt.get(...(search ? [`% ${search}% `, ` % ${search}% `, ` % ${search}% `, ` % ${search}% `] : []));
    const offset = (page - 1) * limit;
    query += ` LIMIT ? OFFSET ? `;
    params.push(limit, offset);
    const stmt = db.prepare(query);
...
export function createSupplier(data) {
    ...
    `);
    const result = stmt.run(data.name, data.code || null, data.contact_person || null, data.email || null, data.phone || null, data.address || null, data.city || null, data.state || null, data.country || null, data.postal_code || null, data.latitude || null, data.longitude || null, data.tax_id || null, data.website || null, data.payment_terms || null, data.notes || null, data.thumb_url || null, data.gallery_items ? JSON.stringify(data.gallery_items) : null, data.is_active ? 1 : 0);
    return { id: result.lastInsertRowid, ...data };
}
...
export function createCustomer(data) {
    ...
    `);
    const result = stmt.run(data.name, data.code || null, data.contact_person || null, data.email || null, data.phone || null, data.address || null, data.city || null, data.state || null, data.country || null, data.postal_code || null, data.latitude || null, data.longitude || null, data.tax_id || null, data.website || null, data.payment_terms || null, data.notes || null, data.thumb_url || null, data.gallery_items ? JSON.stringify(data.gallery_items) : null, data.is_active ? 1 : 0, data.customer_type || 'individual', data.credit_limit || 0, data.outstanding_balance || 0, data.company || null, data.sales_route_id || null);
    return { id: result.lastInsertRowid, ...data };
}
...
// ---------------------------------------------------------
export function getAllPurchaseInvoices({ page = 1, limit = 10, search = '' } = {}) {
    if (!db)
        throw new Error('Database not initialized');
...
export function getPurchaseInvoiceById(id) {
    ...
    const query = `
        ...
        WHERE pi.id = ?
    `;
    const invoice = db.prepare(query).get(id);
    if (!invoice)
        return null;
    ...
    const itemsStmt = db.prepare(`
        ...
        WHERE poi.purchase_order_id = ?
    `);
    const items = itemsStmt.all(invoice.purchase_order_id);
...
export function createPurchaseInvoice(data) {
    ...
    return db.transaction(() => {
        ...
        const po = db.prepare('SELECT * FROM purchase_orders WHERE id = ?').get(data.purchase_order_id);
        if (!po)
            throw new Error(`Purchase Order ID ${data.purchase_order_id} not found`);
        const stmt = db.prepare(`
        ...
        if (!data.invoice_number) {
            const max = db.prepare('SELECT MAX(id) as m FROM purchase_invoices').get();
            const nextNum = (max.m || 0) + 1;
            data.invoice_number = `INV-${new Date().getFullYear()}-${nextNum.toString().padStart(4, '0')}`;
        }
        const result = stmt.run(data.invoice_number, data.purchase_order_id, data.total_payable_amount || 0, data.paid_amount || 0, data.due_amount || (data.total_payable_amount || 0), data.status || 'draft', data.invoice_date || new Date().toISOString(), data.due_date || null, data.notes || null, data.created_by || null);
        return getPurchaseInvoiceById(result.lastInsertRowid);
    })();
}
export function updatePurchaseInvoice(id, data) {
    if (!db)
        throw new Error('Database not initialized');
    const updates = [];
    const params = [];
